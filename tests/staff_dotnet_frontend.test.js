...
const staffViewNav = index.match(/<nav class="view-tabs"[\s\S]*?<\/nav>/)?.[0] || '';
assert.ok(staffViewNav, 'staff view navigation should remain present');
assert.doesNotMatch(staffViewNav, /data-view="profile"/, 'Profile should not be a primary staff view tab');
const sessionActions = index.match(/<div id="session-actions"[\s\S]*?<\/div>/)?.[0] || '';
assert.match(sessionActions, /id="profile-trigger"/, 'Profile should be reachable from the signed-in header actions');
assert.match(workflow, /profileTrigger/);
assert.match(workflow, /profileActive/);
