...
staffUser = { get: key => ({ role: 'super_admin', libraryOrgId: '', username: 'root' })[key] || '' };
body = { barcode: 'b1', title: 'Title', format: 'book', identifier: '978', libraryOrgId: '10' };
t = event();
adminRoutes.staffCreateSuggestion(t.e);
...
staffUser = { get: key => ({ role: 'admin', libraryOrgId: '20', libraryOrgName: 'Library 20', username: 'admin' })[key] || '' };
body = { barcode: 'b2', title: 'Title', format: 'book', identifier: '978' };
t = event();
adminRoutes.staffCreateSuggestion(t.e);
assert.strictEqual(t.response().status, 201);
...
staffUser = { get: key => ({ role: 'super_admin', libraryOrgId: '', username: 'root' })[key] || '' };
body = { barcode: 'b1', title: 'Title', format: 'book', libraryOrgId: '99' };
