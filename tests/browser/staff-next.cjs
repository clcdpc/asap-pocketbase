  ...
  for (const [name, viewport] of [['desktop', { width: 1280, height: 900 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    try {
      ...
      assert.equal(
        ...
        '/api/asap/staff/sign-in?returnUrl=%2Fstaff-next%2F'
      );
...
async function runSuperAdmin(browser, args, axeSource, report) {
  ...
  page.on('pageerror', error => errors.push(error.message));
  try {
    await page.goto(`${args.baseOrigin}/staff-next/?request=${encodeURIComponent(args.legacyRequestId)}`, { waitUntil: 'networkidle' });
    await page.locator('#workspace').waitFor({ state: 'visible' });
...
async function runAnalytics(browser, args, axeSource, report) {
  ...
  try {
    await superPage.goto(`${args.baseOrigin}/staff-next/?stage=analytics`, { waitUntil: 'networkidle' });
  ...
  try {
    await staffPage.goto(`${args.baseOrigin}/staff-next/?stage=analytics`, { waitUntil: 'networkidle' });
    ...
    const cases = [
      {
        ...
        pageUrl: `${args.baseOrigin}/staff-next/?request=${args.staleTitleAId}`,
      ...
      },
      {
        ...
        pageUrl: `${args.baseOrigin}/staff-next/?stage=additional_copies&request=${args.staleCopyAId}`,
...
async function runOrdinaryTitleAssignment(browser, args, report) {
  ...
  for (const [name, viewport] of [['desktop', { width: 1280, height: 900 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    page.on('pageerror', error => errors.push(error.message));
    try {
      await page.goto(`${args.baseOrigin}/staff-next/?request=${args.staleTitleAId}`, { waitUntil: 'networkidle' });
      await page.locator('#request-dialog[open]').waitFor();
...
async function runStaleMutationCompletions(browser, args, report) {
  ...
  try {
    ...
    const copyPage = id => `${args.baseOrigin}/staff-next/?stage=additional_copies&request=${id}`;
    ...
    await page.goto(`${args.baseOrigin}/staff-next/?request=${args.staleCreateSourceId}`, { waitUntil: 'networkidle' });
...
async function runStaleOperationErrorCompletion(browser, args, report) {
  ...
  page.on('pageerror', error => errors.push(error.message));
  try {
    await page.goto(`${args.baseOrigin}/staff-next/?request=${args.resolutionRequestId}`, { waitUntil: 'networkidle' });
...
async function runAdditionalCopies(browser, args, axeSource, report) {
  ...
  page.on('pageerror', error => errors.push(error.message));
  try {
    await page.goto(`${args.baseOrigin}/staff-next/?request=${args.copySourceRequestId}`, { waitUntil: 'networkidle' });
    await page.locator('#request-dialog[open]').waitFor();
    ...
    assert.equal(
      ...
      `/staff-next/?request=${args.copySourceRequestId}`
    );
    page.once('dialog', dialog => dialog.accept());
    await page.getByRole('button', { name: 'Close task' }).click();
...
async function runScopedBlocked(browser, args, axeSource, report) {
  ...
  page.on('pageerror', error => errors.push(error.message));
  try {
    await page.goto(`${args.baseOrigin}/staff-next/?request=${args.blockedRequestId}`, { waitUntil: 'networkidle' });
    await page.locator('#request-dialog[open]').waitFor();
    await page.getByText('Hold placement recovery', { exact: true }).waitFor();
