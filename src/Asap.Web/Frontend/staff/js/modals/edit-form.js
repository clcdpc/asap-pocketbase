...
export function renderPurchaseReminderOption(actionStr, ctx) {
  ...
  checkbox.checked = !!ctx.staffSession.staff?.purchase_reminder_default;
  const isPurchaseAction = actionStr === 'purchase';
  const isStaff = !!(ctx.staffSession.authenticated && ctx.staffSession.accessAllowed && ctx.staffSession.staff);
