...
- **Any Registered Card Login**: In **Settings > Patron Experience**, a library can enable **Allow login from any registered Polaris card**. This is a system-default workflow setting with library overrides. It only applies when the patron portal or New Suggestion staff flow has a specific library context, such as a launch URL with `?libraryOrgId=<ID>` or a saved patron context. When enabled for that launched/effective library, patrons from other Polaris home libraries can authenticate, but suggestions and workflow limits belong to the launched/effective library. The patron’s Polaris home library remains preserved separately, and the launched library context is session-bound rather than a permanent reassignment of the shared patron record. Patron session contexts expire after 24 hours by default; set `ASAP_PATRON_CONTEXT_TTL_HOURS` to a value from 1 to 168 to adjust the TTL.
