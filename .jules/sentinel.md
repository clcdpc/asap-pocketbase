...
## 2026-07-17 - DOM Safety refactoring\n**Vulnerability:** Unsafe `innerHTML` usage found in edit-form.js when constructing anchor tags, violating the project's DOM safety policies.\n**Learning:** Even when variables are wrapped in `escapeAttr()`, building HTML strings manually increases the risk of XSS if the string context changes or interpolation happens without escaping.\n**Prevention:** Use `document.createElement` and `textContent` along with `replaceChildren()` instead of `.innerHTML` assignment, specifically avoiding injection of external or template string variables.
