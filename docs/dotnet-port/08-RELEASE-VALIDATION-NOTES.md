...
- email tests prove authorization-sensitive queued staff messages suppress after recipient deactivation/move/demotion, library deactivation, or applicable destination change; failed rows retain payload beyond 90 days and can be manually retried; terminal sent/suppressed payload is purgeable; missing mail/sender configuration never rolls back ordinary business state;
