...
Authorization scope changes remain lifecycle mutations with atomic cleanup and StaffUser serialization. Staff cookies retain StaffUser ID, normalized authentication email, and sign-in tenant; every request compares the current email and loaded tenant allowlist. Authentication-email changes invalidate old cookies, while OID-only metadata changes do not.
...
## Schema-6 pre-release reset note

Schema 6 is a pre-release reset boundary. Databases created with the earlier OID-based staff identity schema are recreated from the schema-6 DACPAC rather than upgraded in place. Release validation starts from that fresh database and confirms the final email-based EmailOutbox shape; no legacy OID columns, queued-message migration, or authorization fallback is retained.
...
- migration tests/reconciliation pass, including valid unique source staff emails without an identity map, null initial Entra metadata, recipient/preference preservation, and the pre-start usable email-authenticated super-admin gate/bootstrap path;
...
- staff-auth tests prove a cookie is rejected immediately after the same StaffUser's authentication email changes, while changed OID metadata with the same email preserves authorization and refreshes last-observed metadata;
...
For production-hostname preflight, use a disposable SQL database/config only. Preflight must validate real Entra `tid`/`oid` claims, allowed-tenant enforcement, normalized-email local authorization, and other host-dependent behavior without creating any runtime/bootstrap rows in the final PocketBase-migration target. Destroy the disposable database and keep the application stopped against the recreated final target until import/reconciliation completes.
