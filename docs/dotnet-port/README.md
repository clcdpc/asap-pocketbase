...
Schema 6 is a pre-release reset boundary. Databases created with the earlier OID-based staff identity schema are recreated from the schema-6 DACPAC rather than upgraded in place. The schema contains only the final email-identity model, and normal DACPAC data-loss protection remains enabled.
