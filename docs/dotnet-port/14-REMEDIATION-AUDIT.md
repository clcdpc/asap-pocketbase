...
- Pack `README.md` reports this cleanup. `examples/Config.example.json` adds only an explicitly allowed example subdomain; existing keys and unrelated example values are unchanged. The manifest retains its self-hash exclusion and inventories the 19 pack payload files; repository-root `AGENTS.md` is outside that payload.
