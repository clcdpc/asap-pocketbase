...
- Entra OIDC + local StaffUser allowlist/roles; normalized email is the local authorization identity. Tickets retain StaffUser ID, normalized email, and sign-in tenant. Stored `tid`/`oid` are last-observed metadata only; authentication-email changes invalidate cookies while OID changes do not.
...
- Durable SQL email outbox + Hangfire with exactly `pending`/`sending`/`sent`/`failed`/`suppressed`: every expired `sending` lease is potentially transport-ambiguous; the provider call starts within 30 seconds of claim and has a 30-second complete-operation timeout; the lease is two minutes; reclaim/retry begins only after expiry; and final writes compare `Status`, expected `LeaseId`, and post-claim rowversion/equivalent ownership so late workers cannot overwrite newer attempts. Preserve explicit at-least-once semantics with no exactly-once claim, `failed` manual retry with payload retained, payload purge only for terminal `sent`/`suppressed` after 90 days, and missing enqueue-time notification configuration represented non-fatally rather than rolling back business state. Authorization-sensitive staff mail stores recipient StaffUser ID, normalized authentication-email snapshot, authorization scope, and `RecipientAddressKind=notification_email|weekly_summary`; it revalidates current email/allowed-tenant/authorization plus the kind-specific effective address against `ToAddress` immediately before every send/retry, with stale authorization/address becoming terminal suppression.
...
- Migration imports valid unique source staff emails directly and leaves Entra metadata null until sign-in; no identity map is used. Before startup, hard-gate on an active email-authenticated super-admin and provision/promote by configured bootstrap email when required. All existing settings, recipient, claimant, identifier, and fresh-target reconciliation rules remain in force.
...
3. Use current StaffUser ID/email/tenant/activity/scope evidence consistently; sensitive mail snapshots authentication email. No Graph, session store, or key-rotation workaround.
...
5. Perform the full permanent-nonproduction PocketBase -> SQL migration and IIS deployment rehearsal using those exact artifacts, including source staff-email validation and fresh-target checks.
