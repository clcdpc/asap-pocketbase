...
3. Record the starting PocketBase SHA. Do not prematurely create a tag called "final" if urgent PocketBase fixes could still land; create/verify the permanent final-PocketBase tag on the last PocketBase `main` commit immediately before the .NET PR replaces `main`.
...
- Durable SQL email outbox + Hangfire with exactly `pending`/`sending`/`sent`/`failed`/`suppressed`: lease-based `sending` claims, stale-lease recovery, explicit at-least-once transport ambiguity, `failed` manual retry with payload retained, payload purge only for terminal `sent`/`suppressed` after 90 days, and missing enqueue-time notification configuration represented non-fatally rather than rolling back business state. Authorization-sensitive staff mail stores recipient/scope/original Entra tuple and revalidates current binding/allowed-tenant/authorization/address immediately before every send/retry; stale authorization becomes terminal suppression.
...
- Staff recipient contract: `NotificationEmail` is the nullable primary ordinary-notification destination; `WeeklyActionSummaryEmail` is a weekly-summary-only optional override/fallback. Reject legacy `@staff.asap.local` placeholders, preserve migration precedence, permit authorized Staff Access to clear the primary email, never repopulate it from Entra sign-in, and emit migration reconciliation deltas for newly eligible weekly-summary users and every ordinary-recipient change. The target fallback normalization is deliberate.
...
1. Confirm the PocketBase production/main line has no urgent fix missing from the port; create/verify the permanent **final PocketBase** tag on that last PocketBase commit.
