...
Schema 6 is a pre-release hard-reset boundary. Before the first schema-6 deployment, recreate any test or application database built from the earlier OID-based staff identity schema and deploy the schema-6 DACPAC to that fresh target. Do not attempt an in-place upgrade or preserve queued OID-based outbox rows. The first deployment that introduces `TitleRequest.BibIdStaffVerified` also requires a fresh, subsequently imported target; applying its `DEFAULT (0)` to populated pre-column request rows cannot recover BIB authority. Import enforces target freshness. After import, later DACPAC releases preserve the stored flag values. Normal deployments retain `BlockOnPossibleDataLoss=True` and `DropObjectsNotInSource=False`.
...
- validate real hostname HTTPS binding/certificate, Entra redirect/issuer and allowed-tenant enforcement, normalized-email staff authorization, validated `tid`/`oid` metadata capture, cookies, antiforgery, patron CSP/embedding, static assets, deep links, staff login, and health without changing public DNS;
