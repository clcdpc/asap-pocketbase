...
- Create a permanent Git tag identifying the final PocketBase implementation immediately before the .NET port replaces `main`; it must point to the last PocketBase production/main commit after any urgent freeze-period fixes.
...
- Each slice gets at least two and at most three review passes. Blocking correctness/security/data-integrity/migration/material-regression findings always block progression regardless of nominal cap.
...
- Distinguish immutable `business_event` from `staff_authorization_sensitive` mail. Sensitive rows persist RecipientStaffUserId, recipient Entra tuple, authorization scope, and intended address; immediately before every send/retry apply the common current binding/allowed-tenant/activity/role/scope/participation predicate and require current effective address equality. Otherwise terminally suppress without sending. Immutable business-event mail may drain after participation changes without gaining this staff dependency.
...
- Commit business transaction/outbox intent first, then enqueue Hangfire; a sweeper repairs enqueue gaps. Workers atomically claim due rows with `Status=sending`, a unique lease ID, and lease expiration before calling Postmark; expired leases recover to retryable work so rows cannot strand permanently.
- Email transport is explicitly at-least-once: a crash after Postmark accepts a message but before local success is committed may cause a duplicate retry. Business/outbox idempotency prevents duplicate local notification creation but cannot guarantee provider exactly-once semantics.
...
- After cutover keep PocketBase stopped/offline and not normally reachable; retain executable/data/config for about 30 days so an administrator can start it manually for investigation if needed.
- Retain the final PocketBase backup/tag as the durable historical reference per operations policy.
...
11. verify/create final PocketBase tag -> merge port -> create production version tag -> exact-artifact permanent-nonproduction rehearsal -> production cutover.
...
- Keeping old PocketBase app continuously reachable read-only after cutover — refined to stopped/offline, manually startable for temporary investigation.
