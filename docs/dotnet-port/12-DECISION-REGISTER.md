...
- Keep the port in `clcdpc/asap-pocketbase` on one dedicated port branch and one large draft PR.
- Implement internally as complete vertical slices; do not treat slices as separately mergeable releases.
...
- Transition: Slices 0-4 are accepted; preserve their actual Sol/Luna Max execution and review evidence. The roles below apply beginning with Slice 5, which has not started (`../implementation/PORT-STATUS.md`).
...
- GPT-5.6 Luna High: default primary implementer, normally with a fresh context per remaining slice. Owns the complete ordinary C#, SQL/DACPAC, migration/export/import/reconciliation, frontend, tests, test-failure diagnosis, directly affected documentation, and confirmed Terra fixes/retesting.
- GPT-5.6 Luna Max: bounded implementation escalation chosen by Astra for a concrete problem that materially benefits from additional reasoning under document 10. Prefer returning to Luna High afterward; size, SQL, authentication, migration, test volume, or elapsed time alone do not justify escalation.
- Keep Luna context while efficient/useful; Astra may rotate to fresh Luna High when accumulated context becomes materially large/repetitive. Use document 10's concise handoff with exact SHA, remaining work/invariants, test status, evidence paths and next action; the new context inspects current code before editing. Context identity is not an acceptance invariant.
- GPT-5.6 Terra High: independent reviewer, fresh context per slice for full Pass 1 and retained for required focused/full re-review; fresh contexts for the separate final whole-app review. Review correctness/regression, callers/surrounding code, state/data, SQL/migrations, authorization/scope, concurrency/ordering, recovery, external side effects, frontend/API compatibility, test adequacy, and unnecessary complexity/coupling.
- Confirmed findings return to the current Luna implementation context; Terra reports findings only and never implements fixes. Focused re-review covers fixes, affected callers/invariants, regression surface, tests and fix interactions. Document 10 requires a full review for systemic findings, broad/shared changes, material authorization/concurrency/migration architecture changes, several unrelated changed areas, new unrelated substantive defects, or unbounded regression surface.
- Luna -> Astra Max is the mandatory first escalation for materially blocking contract, Terra-finding, invariant, cross-slice, provider, architectural-deviation or slice-boundary/deferral questions under `10-CODEX-MULTI-MODEL-TASK.md`. Luna must not bypass Astra or dispatch Sol directly; no fixed number of Luna failures is required before Astra reasons about an issue.
- Astra owns the complete contract, accepted decisions, cross-slice state, packet scope, current implementation, progression and acceptance. Astra may inspect the authoritative pack, accepted implementation, pinned source and tests/evidence, decide which contract governs, narrow the problem and give Luna a concrete implementation ruling. Resolve directly whenever that evidence suffices; Luna implements/tests and Terra verifies through the existing gate.
- GPT-5.6 Sol High: fresh bounded consultation only when Astra's focused analysis determines that material uncertainty remains or independent/deeper reasoning has material value under `02-IMPLEMENTATION-PLAN.md` section 3 / document 10. Only Astra dispatches Sol. Code size, difficult SQL, migration, concurrency, authentication, configuration, deployment, external integration or a routine Luna question alone does not justify Sol.
- Sol provides root cause, alternatives, the smallest faithful resolution, and affected invariants/tests. Astra evaluates the advice against the authoritative pack and accepted implementation and decides the resolution; the current Luna implementation context implements/tests and Terra independently verifies. Neither Sol High nor Sol XHigh takes slice ownership.
- GPT-5.6 Sol XHigh: exceptional consultation only when Astra determines that bounded Sol High advice still leaves a blocking issue unresolved, or the problem requires exceptional cross-system reasoning. Astra retains the resolution decision. Sol remains outside the normal slice loop and never replaces Astra as Luna's first escalation point.
- Astra refreshes each prepared packet against accepted prior implementation before dispatch. After Luna tests/fixes and the required Terra review, Astra verifies acceptance, creates/pushes the milestone, and requires actual remote CI success for that exact commit before dispatching the next slice.
- Keep detailed logs in retained evidence files. Astra/Luna normally consume compact command/result/count/hash/path/warning receipts, expanding relevant raw sections for failure, mismatch, detail-dependent findings, security/concurrency/migration inspection or ambiguity. Astra verifies required gates ran/passed; do not rerun expensive successful gates solely for duplicate model consumption.
- Astra may resolve ordinary ambiguity from the pack/current app/repository without asking the user.
- Luna implements the existing contract: no unrequired repositories, MediatR, generalized workflow/messaging infrastructure, migration frameworks, layering, or speculative hardening. Context/re-review optimization preserves independent review and every objective build, SQL, Node/frontend, browser/axe, native migration, artifact/DACPAC, exact-SHA CI and applicable release/rehearsal gate.
...
- Recurring schedules live in external environment JSON and require restart to change. The authoritative matrix/defaults are in `01-PORTING-SPEC.md` and must be completely represented in `examples/Config.example.json`; all cron expressions use the configured business timezone.
