...
- Entra proves the user controls an account in an allowed tenant. ASAP locates and authorizes the local StaffUser by normalized email. Entra `tid`/`oid` are retained as last-observed metadata but are not the application's staff authorization key.
- `UserPrincipalName`/`NormalizedUserPrincipalName` hold the canonical authentication email with case-insensitive uniqueness. `DisplayName` may refresh from claims; `NotificationEmail` remains independently app-owned.
- Initial super-admin email/UPN comes from external config and is auto-created only when StaffUser is empty. Legacy tenant/object config is optional and not required for bootstrap authorization.
- New bootstrap templates emit only initial-admin email/UPN plus optional display/contact values. `AllowedTenantIds` separately defines trusted Entra tenants; operators do not provision an OID for bootstrap, and the new row's tenant/object metadata remains null until first successful sign-in.
...
- The auth ticket retains `StaffUserId`, normalized authentication email, and the validated sign-in tenant. Every authenticated request applies the common predicate in `01-PORTING-SPEC.md` section 7.6: exact current email, loaded allowed tenant, active StaffUser, current role/organization/scope, and participation where required. Authentication-email change, deactivation, or tenant removal invalidates old authorization; readable profile and observed OID changes do not. Sensitive mail snapshots and checks the authentication email; usable-admin and candidate checks require an active valid email. Persistent Data Protection keys do not bypass changed tenant trust.
...
- Active staff require a valid normalized authentication email; tenant/object metadata may be null before first login.
- Deactivate staff rather than deleting them. Re-adding the same normalized email reactivates the row while preserving preferences and unrelated metadata, subject to the existing active-library rule.
- At least one currently usable email-authenticated super-admin in Organization 1 must remain. Every mutation reducing that eligibility uses the existing transaction-owned invariant lock and returns 409 if zero would remain.
- Promotion to super-admin atomically changes organization to 1; demotion requires an explicit destination library.
- Library admins can manage staff/admin lifecycle and authentication email within their own library; only super-admins manage super-admins.
...
- Provisioning requires email, role, and library where applicable. Changing authentication email is the audited identity-changing metadata operation; the manual Entra rebind endpoint/UI is removed.
- Do not use Microsoft Graph or directory provisioning machinery. Successful email-matched sign-in automatically updates last-observed tenant/object metadata.
...
- Hold routing is not configuration: `RequestingOrgID` comes from the live patron's registered Polaris organization, while `PickupOrgID` comes from the live eligible default with registered-organization fallback only when eligible. The request pickup fields remain temporary display/audit snapshots, and the exact resolved IDs are frozen at create dispatch for reply/recovery.
- `OrganizationIdForRequests` and `PickupOrganizationId` are retired from the target schema, DTOs, UI, and runtime. PAPI calls made before a patron is known use the documented bootstrap organization only as transport/reference context; it is never used as hold routing data.
...
- One tightly permissioned external JSON file contains pre-application/environment/bootstrap configuration and secrets: connection strings, Entra client ID/tenant allowlist/email-only bootstrap admin, environment-specific Data Protection key-ring path, filesystem paths, Hangfire schedules, environment banner, nonproduction recipient-domain safety, etc. Entra authentication requires no client secret.
...
- Distinguish immutable `business_event` from `staff_authorization_sensitive` mail. Sensitive rows persist `RecipientStaffUserId`, `RecipientAuthenticationEmail`, authorization scope, intended address, and `RecipientAddressKind=notification_email|weekly_summary`. Immediately before every send/retry apply the common current email/allowed-tenant/activity/role/scope/participation predicate, resolve the current address by that explicit kind, and require equality with `ToAddress`; never infer the kind from `BusinessKey`. `notification_email` always uses `NotificationEmail`; `weekly_summary` uses nonblank `WeeklyActionSummaryEmail` and otherwise `NotificationEmail`. Otherwise terminally suppress without sending. Immutable business-event mail may drain after participation changes without gaining this staff dependency.
...
- Staff creation, authentication-email/profile changes, role/library changes, and deactivation are audited through their owning lifecycle operations.
...
- Staff migration uses the real source staff email as authentication identity and imports tenant/object metadata as null. Active missing/invalid/placeholder emails and duplicate normalized emails block import; no identity map is required.
- Before startup, migration requires one active super-admin with a valid authentication email. If needed it matches the configured bootstrap by normalized email, promotes/reactivates that row or inserts one, reports the intervention, and re-runs the gate.
...
- Resolve migrated `NotificationEmail` from the real PocketBase staff email, with valid legacy weekly-summary email as fallback only where permitted for historical inactive rows; reject `@staff.asap.local`. Migrate the weekly override independently and report recipient/eligibility deltas.
...
- Migration source accounting retains legacy Polaris `requestingOrgId`/`pickupOrgId` as intentional drops; no target setting or runtime fallback is created for them.
...
- Before DNS cutover, use a workstation hosts-file override so the real production hostname resolves to the new server and validate TLS, cookies, Entra redirect/issuer, allowed-tenant enforcement, normalized-email staff authorization, `tid`/`oid` metadata capture, CSP/embed behavior, static assets, and deep links under the real URL. Then stop IIS, restore the final config, destroy the disposable preflight DB, and recreate/reset the final target to fresh migration state; do not start against it before import.

- Schema 6 is a pre-release reset boundary. Databases created with the earlier OID-based staff identity schema are recreated from the schema-6 DACPAC rather than upgraded in place. No compatibility columns, queued EmailOutbox migration, OID authorization fallback, or general schema-upgrade framework is introduced; normal DACPAC data-loss protection remains enabled.
