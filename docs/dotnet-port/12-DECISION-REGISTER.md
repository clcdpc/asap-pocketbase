...
Staged PRs begin with Slice 6; thin autonomous supervisor mode becomes preferred/default for the remainder of Slice 6 unless the user explicitly requests manual/direct execution. Existing bootstrap remains valid. Slices 0-5 retain their actual historical execution/review evidence and acceptance. [Document 10](10-CODEX-MULTI-MODEL-TASK.md) is the authoritative execution/model/review policy; [PORT-STATUS.md](../implementation/PORT-STATUS.md) records current state.
...
- Terra High remains an independent bounded reviewer and reports findings only. Detailed package review asks whether the bounded behavior is correct. Fresh holistic slice review assesses the complete delta from prior accepted product milestone to final slice head. Confirmed findings go to bounded Luna fixes, then focused Terra re-review; retaining the same Terra is not an acceptance invariant.
- Astra Max may remain as a thin autonomous slice supervisor operating as a state-machine controller with a strict context firewall. It owns progression, dispatch, branch/PR state, receipt verification, fix routing, short acceptance, integration, milestone records, CI polling and final status. Persistent technical-parent behavior is prohibited: implementation/debugging/detailed review remain with workers. Retain state/SHAs/PRs/receipts/findings/CI, not transcripts/logs/patches/source dumps. Technical reasoning is limited to bounded contract escalation, conflicting reports and narrow acceptance invariants that receipts cannot establish.
