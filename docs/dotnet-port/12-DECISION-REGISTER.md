...
## 3. Direct-model implementation and review

The staged-PR/direct-model policy begins with Slice 6. Slices 0-5 retain their actual historical execution/review evidence and acceptance. [Document 10](10-CODEX-MULTI-MODEL-TASK.md) is the authoritative execution/model/review policy; [PORT-STATUS.md](../implementation/PORT-STATUS.md) records current state.

- Luna Max runs directly as normal implementation owner for small slices, packages, integration/glue and confirmed fixes, including ordinary C#, SQL/DACPAC, migration, frontend, tests, debugging and affected documentation. Luna High is optional, never a required first tier.
- Terra High runs directly and reports findings only. Detailed package review asks whether the bounded behavior is correct. Fresh holistic slice review asks whether the components form a correct complete slice, from prior accepted milestone to final slice head. Confirmed findings return directly to Luna Max; review receipts guide focused re-review without masking new concerns.
- Astra Max has short bootstrap, separate material-contract consultation and acceptance roles. It stops after boundary decisions instead of remaining a parent during implementation/review. Ordinary decisions stay in direct tasks; unresolved product/security/migration/architecture questions pause the task for a concise separate Astra ruling.
...
- Use branch/PR diffs, relevant instructions/packets/contracts and compact receipts. Expand sources for concrete questions; do not reread the full pack per package. Preserve required local-only evidence across context rotation; handoffs record SHA, state, findings, validation and next action.
- Package validation is proportionate; integrated slice validation runs the complete existing matrix. Review fixes rerun affected gates and broaden only when prior evidence is invalidated. Astra verifies current receipts without repeating successful work; the exact integration milestone's remote CI completes acceptance.
