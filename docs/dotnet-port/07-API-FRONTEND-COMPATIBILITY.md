...
5. Callback requires validated Entra `tid` + `oid`, verifies the tenant is allowed, resolves a valid email claim (falling back to valid `preferred_username`), and finds the active `StaffUser` by `NormalizedUserPrincipalName`. The ticket records StaffUser ID, normalized authentication email, and sign-in tenant. Every request reloads the row, rechecks email equality and current tenant allowance, and applies current role/library/participation rules. Stored OID is never compared.
6. On successful sign-in the server records the current `tid`/`oid` as last-observed metadata, refreshes `DisplayName` where available, and updates last login. It does not change authentication or notification email.
