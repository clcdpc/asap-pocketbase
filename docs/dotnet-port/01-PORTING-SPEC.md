...
Distinguish immutable business-event messages from authorization-sensitive staff messages. Patron/business-event notifications may drain after later library participation changes because they represent an already-committed event. Assignment notices, purchase/additional-copy reminders, weekly summaries, and any other staff message containing library-restricted request/task data are `staff_authorization_sensitive`. Persist the recipient `StaffUserId`, authorization Organization/scope, and an explicit `RecipientAddressKind` for the snapshotted content. The only required address kinds are `notification_email` for ordinary staff notifications and `weekly_summary` for ordinary or forced weekly summaries; never infer the rule from `BusinessKey`, template, or payload text. Immediately before **every send or retry**, re-read the recipient and revalidate current interactive authorization for that scope: StaffUser must still be active; a library-scoped message requires the referenced library to remain active and the recipient to still have access to that library (same-library staff/admin or still-valid super-admin scope); a consortium-wide summary requires the recipient to remain an active super-admin. Resolve the current destination according to the stored kind: `notification_email` uses `NotificationEmail`, while `weekly_summary` uses nonblank `WeeklyActionSummaryEmail` and otherwise falls back to `NotificationEmail`. Compare that current effective destination with the snapshotted `ToAddress` using the same address normalization applied at intent creation. If authorization or destination ownership has changed, atomically transition the row to terminal `suppressed` with a safe reason and do not send the old snapshot.
...
Recurring jobs cover the current automation set plus target-only recovery/maintenance work. The following table is the authoritative default schedule contract; the operational template embedded in `scripts/deployment/Initialize-AsapTestHost.ps1` uses the same keys and defaults. Cron expressions are interpreted in the configured `Application.BusinessTimeZone` (`America/New_York` by default), not server-local time or UTC unless explicitly configured otherwise.
