...
`BibIdStaffVerified` protects a BIB deliberately selected and validated by the target staff workflow from identifier-processor replacement or clearing. Background lookup results remain unverified. Migration must apply the source evidence, retry/reopen risk checks, and explicit authority reporting in `04-MIGRATION-CUTOVER.md` section 6.9. A `closed` request with a retryable identifier state can become a worker candidate through `reopen`; imported ambiguous BIBs in that state require placed-history protection or source correction. Identifier edits invalidate the prior BIB and its authority before a new lookup.
