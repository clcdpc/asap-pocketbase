...
- Creating an account requires email, role, and library where applicable. Changing the authentication email uses the ordinary authorized metadata mutation with uniqueness, row-version, scope, and audit enforcement; there is no manual Entra rebind operation.
