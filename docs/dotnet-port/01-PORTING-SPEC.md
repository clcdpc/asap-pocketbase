...
The port stays in the existing `clcdpc/asap-pocketbase` repository on a dedicated branch and one large draft PR. Once the .NET deployment has been validated in production, rename the repository to `clcdpc/asap`. Create a permanent Git tag for the final PocketBase implementation before the port replaces `main`.
...
- The durable authorization identity key is **(`EntraTenantId`, `EntraObjectId`)**. Never authorize or locate an account by UPN, `preferred_username`, or email because those values are human-readable but mutable/reassignable.
- Preserve database readability beside the durable key: store the latest known `UserPrincipalName`/preferred username, `DisplayName`, and app-owned `NotificationEmail`. These fields are searchable/displayable but are not authorization keys.
- On successful sign-in, refresh readable Entra profile fields such as `UserPrincipalName` and `DisplayName` from validated claims without changing the durable tenant/object binding. **Never initialize, repopulate, or overwrite `NotificationEmail` from Entra claims during sign-in.** It is app-owned contact data and may be null by deliberate administrative choice.
- No Microsoft Graph lookup/existence validation is required. Staff provisioning therefore requires the administrator to supply the allowed tenant ID, Entra object ID, and a readable UPN/email label; an incorrect object ID simply cannot authenticate.

Normal Visual Studio F5 uses real Entra/OIDC. There is no interactive Development authentication bypass.

### 7.2 Bootstrap

The external configuration specifies the initial super-admin's Entra tenant ID, Entra object ID, and human-readable UPN/email (plus optional display/notification values). Only when the `StaffUser` table is empty may **normal startup/bootstrap** create that exact bound account. There is no setup page and no first-login binding by mutable UPN/email. Subsequent access requires an existing active local `StaffUser`. The one-time migration is different: after StaffUser import and before the web application or Hangfire is enabled, migration must hard-gate on at least one active `super_admin` with a valid allowed-tenant (`EntraTenantId`,`EntraObjectId`) binding. If import yields none, `Asap.Migration` must explicitly provision/promote the configured bootstrap identity during the stopped-app cutover, report that intervention, and re-run the gate; it must never rely on normal startup bootstrap after other StaffUser rows already exist.
...
- The authentication ticket stores the validated `StaffUserId` and Entra `tid`/`oid` that established the session. On **every authenticated staff request**, reload the row and apply the common current eligibility predicate in section 7.6, including membership of `tid` in the **currently loaded** `AllowedTenantIds`. An inactive account, old identity binding, or removed tenant rejects the principal and expires the cookie (`401` JSON for APIs); a valid identity lacking current role/resource/participation permission receives `403`. An old cookie is not grandfathered through a policy change merely because the persistent Data Protection ring can decrypt it. Readable profile refresh alone does not invalidate a cookie.
...
- At least one **usable** super-admin must remain: apply section 7.6 to the row's current binding with required system/super-admin scope. Demotion, deactivation, identity rebind, or any other StaffUser mutation that could reduce that count takes the transaction-owned exclusive SQL application lock `ASAP:ActiveSuperAdminInvariant` **before** the normal row-lock hierarchy, re-reads the prospective invariant, and either commits or returns `409 Conflict` (`usable_super_admin_required`). A bounded lock timeout is a retryable conflict, never permission to skip the check. External tenant-policy changes use the stopped-app validation/startup gate in section 7.6; a count of merely active role rows is insufficient.
- Library admins can manage staff/admin accounts in their own library, including readable profile values, durable Entra identity binding, demotion, and deactivation; only super-admins manage super-admin accounts.
...
- Library admins may assign an identity from any globally allowed Entra tenant; membership in the admin's library is still determined by the local `StaffUser.OrganizationId`.
- Creating/rebinding an account requires explicit `EntraTenantId` + `EntraObjectId` and a readable UPN/email label. Rebinding the durable identity is a high-impact, explicitly confirmed and audited action; never infer or auto-rebind from UPN/email changes.
- `UserPrincipalName` and `DisplayName` are readable Entra profile metadata and may refresh from validated claims on sign-in. `NotificationEmail` is separate application-owned contact data and changes only through bootstrap/provisioning, migration, or an authorized Staff Access mutation.
- Deactivation uses `IsActive = false`, never hard deletion.
- Re-adding the same (`EntraTenantId`, `EntraObjectId`) reactivates the existing record. For `staff`/`admin`, the referenced non-system library must be active (or the administrator must first move the account to an active library); otherwise reject the reactivation mutation with `409 Conflict`. Organization `1` is permanently active for `super_admin`. A recycled UPN/email with a different object ID is a different identity.
...
`StaffUser` keeps application identity/profile data only: durable Entra tenant/object IDs, readable UPN/preferred-username text, display name, notification email, role, organization, active state, current per-user workflow/profile preferences, and last-login timestamp. Preserve the current preference set: `WeeklyActionSummaryEnabled`, `WeeklyActionSummaryEmail`, `PurchaseReminderDefault`, `AdditionalCopyReminderDefault`, and `DefaultMineUnclaimedFilter`. These are user preferences, not system/library settings. Drop PocketBase/Polaris-specific identity fields. `UserPrincipalName`/`DisplayName` may refresh from validated Entra claims; `NotificationEmail` is changed only by bootstrap/provisioning/migration or an authorized Staff Access mutation and is never repopulated by later Entra sign-in.
...
Use one common eligibility function with explicit identity evidence, required role/resource scope, and a `requireParticipation` context. It must require: an existing active StaffUser; a complete current durable `(EntraTenantId, EntraObjectId)` binding; exact equality of the supplied identity tuple with that binding; membership of its tenant in the loaded `AllowedTenantIds`; a valid current role/Organization combination and access to the requested scope; and active Organization participation wherever that action/content requires it. Read role and Organization from current SQL state, never stale cookie claims. No Graph call, staff-session database, or Data Protection key rotation is involved.

Use the same function at OIDC completion, on every authenticated staff request, immediately before every authorization-sensitive staff send/retry, for usable-super-admin counts, and for prospective external-configuration validation. Identity evidence is the original ticket tuple for a request and the recipient tuple persisted at intent creation for staff mail; those must still match. For a new relationship or usable-admin/config check with no prior ticket, evaluate the row's current explicit binding itself, not a tuple inferred from UPN/email. The mail address must also pass the existing message-class destination check. The function does not turn a background job into a staff session.

**Stored claim activation:** whenever a claimant becomes effective on open operational work (including an inherited claim or reopening without changing the FK), evaluate current StaffUser eligibility for that work's library with `requireParticipation = false`. This validates the active account, binding/trust, and same-library staff/admin or cross-library super-admin scope, but does not clear a valid stored relationship merely because its Organization is inactive. Participation remains a separate gate on operating that work. Existing closed history is not rewritten, and tenant-policy changes do not trigger a new bulk claim-cleanup mechanism. At migration, active staff identity/trust validation already precedes claim normalization; map first and validate the resulting target claimant separately.
...
Distinguish immutable business-event messages from authorization-sensitive staff messages. Patron/business-event notifications may drain after later library participation changes because they represent an already-committed event. Assignment notices, purchase/additional-copy reminders, weekly summaries, and any other staff message containing library-restricted request/task data are `staff_authorization_sensitive`. Persist the recipient `StaffUserId` and authorization Organization/scope required for that snapshotted content. Immediately before **every send or retry**, re-read the recipient and revalidate current interactive authorization for that scope: StaffUser must still be active; a library-scoped message requires the referenced library to remain active and the recipient to still have access to that library (same-library staff/admin or still-valid super-admin scope); a consortium-wide summary requires the recipient to remain an active super-admin. Also require the current effective app-owned staff notification address for that message class to still equal the snapshotted `ToAddress`. If authorization or destination ownership has changed, atomically transition the row to terminal `suppressed` with a safe reason and do not send the old snapshot.

Delivery workers claim one `pending` row atomically by moving it to `sending` with a unique lease token and lease-expiration timestamp, then commit before calling Postmark. No SQL transaction is held across the network call. A worker marks success `sent`, returns a transient failure to the bounded retry path, or eventually records `failed`. The sweeper also detects **expired `sending` leases**: a claim that expired before a provider call can be safely retried, while a claim that may have reached Postmark is treated as an ambiguous at-least-once delivery and retried only under the documented grace/retry policy. Database-enforced business/outbox idempotency prevents duplicate local messages, but the transport cannot promise exactly-once delivery if a process dies after Postmark accepts a message and before ASAP records success.
...
Recurring jobs cover the current automation set plus target-only recovery/maintenance work. The following table is the authoritative default schedule contract; `examples/Config.example.json` uses the same keys and defaults. Cron expressions are interpreted in the configured `Application.BusinessTimeZone` (`America/New_York` by default), not server-local time or UTC unless explicitly configured otherwise.
