...
- StaffUser active-email requirements and filtered unique normalized-email index; verify case-insensitive duplicate rejection, null initial tenant/object metadata, first-login metadata capture, same-email changed-OID success, wrong-email rejection, and old-cookie invalidation after authentication-email change;
...
- analytics queries;
- migration import/reconciliation, including valid unique source staff emails with null Entra metadata, operational configuration parity, recipient deltas, preference preservation, the usable email-authenticated super-admin gate/bootstrap path, and auto-claim-rule assignee normalization;
...
- hard-failure tests for active staff missing/invalid/placeholder emails and duplicate normalized emails, plus unknown actor types, unresolved formats, bad FKs, and invalid branding;
- migration-time usable-super-admin tests: a valid imported email-authenticated super-admin passes; otherwise the configured bootstrap email promotes/reactivates a matching row or inserts exactly one row and reports it;
- migration tests proving active valid unique source emails import without an identity map and tenant/object metadata remains null until sign-in;
...
For **both** TitleRequest and AdditionalCopy, import unmapped, inactive, wrong-library, demoted-super-admin, valid same-library, and valid cross-library super-admin claimants; test open/actionable and closed counterparts. Active staff email errors still block the separate identity gate.
