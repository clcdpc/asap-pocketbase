...
- email outbox idempotency with a filtered unique non-null `BusinessKey`, simultaneous duplicate enqueue races resolving to one row/successful idempotent results, normal recipient+period weekly-summary idempotency, forced-summary `ManualRunId` keys, lease acquisition, expired-`sending` recovery, terminal `suppressed` state, authorization-sensitive recipient/scope fields, and payload-purge eligibility limited to terminal `sent`/`suppressed`;
...
- worker crashes after outbox claim but before provider call -> expired lease is recovered without permanent `sending` state;
...
- authorization-sensitive queued staff mail: queue an assignment/reminder/weekly message, then before delivery deactivate the recipient, move them to another library, demote a cross-library super-admin, deactivate the authorization library, and separately clear/change the applicable staff notification address; each case suppresses the old row and sends no restricted snapshot; an immutable patron/business-event row still drains when intended;
...
- simulated crash after provider acceptance but before local `sent` commit -> row is recoverable and the test/documentation explicitly acknowledges possible duplicate delivery under at-least-once semantics;
