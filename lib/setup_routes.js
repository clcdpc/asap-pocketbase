...
function polarisConfigured(app) {
  var p = config.polaris(app);
...
function setupStatus(e) {
  ...
  return e.json(200, {
    ...
    polarisConfigured: polarisConfigured(e.app)
  });
}
...
function initialSetup(e) {
  ...
  try {
    var setupStaffAuth = polaris.adminStaffAuth(polarisData);
    orgSync = orgs.syncOrganizations(e.app, setupStaffAuth);
    try {
      patronCodes.syncPatronCodes(e.app, setupStaffAuth, polarisData);
    } catch (patronCodeSyncErr) {
      e.app.logger().warn("Polaris patron code sync failed during setup", "error", String(patronCodeSyncErr));
    }
