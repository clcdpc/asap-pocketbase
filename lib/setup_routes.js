...
function setupStatus(e) {
  ...
  var hasPolaris = polarisConfigured();
  return e.json(200, {
    setupRequired: !hasStaff || !hasPolaris,
    hasStaffUsers: hasStaff,
    polarisConfigured: hasPolaris
  });
}

function initialSetup(e) {
  var hasStaff = records.hasStaffUsers(e.app);
  var recoveryMode = hasStaff && !polarisConfigured();
  if (hasStaff && !recoveryMode) {
  ...
  var staffIdentity = null;
  if (!recoveryMode) {
    staffIdentity = identity.parseStaffIdentity(data.adminUsername || "", polarisData.staffDomain);
  ...
  if (recoveryMode) {
    return e.json(200, {
      recoveredPolaris: true,
      message: "Polaris settings were saved. Staff login is available again.",
