...
function enforceDuplicate(app, barcode, data, libraryOrgId) {
  ...
  let libraryScoped = String(libraryOrgId || "").trim();
  if (libraryScoped) params.libraryOrgId = libraryScoped;

  if (identifier) {
    let isbnExisting = app.findRecordsByFilter(
      "title_requests",
      libraryScoped ? "barcode = {:barcode} && libraryOrgId = {:libraryOrgId} && identifier = {:identifier}" : "barcode = {:barcode} && identifier = {:identifier}",
      "-created",
      1,
      0,
      libraryScoped ? { barcode: barcode, libraryOrgId: libraryScoped, identifier: identifier } : { barcode: barcode, identifier: identifier }
    );
  ...
  let filter = libraryScoped ? "barcode = {:barcode} && libraryOrgId = {:libraryOrgId} && ((title = {:title} && format = {:format})" : "barcode = {:barcode} && ((title = {:title} && format = {:format})";
  params.title = title || "";
