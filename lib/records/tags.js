...
function removeWorkflowTagForRequest(app, record, tag) {
  ...
  try {
    ...
    if (rows.length > 0) {
      if (app.db && typeof app.db === "function") {
        try {
          app.db().newQuery("DELETE FROM title_request_tags WHERE titleRequest = {:request} AND tag = {:tag}").bind({ request: record.id, tag: tagRecord.id }).execute();
          removed = true;
        } catch (err) {
          // Fallback to iteration on bulk query failure
