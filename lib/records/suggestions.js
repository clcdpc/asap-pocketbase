...
const transactions = require("../pb_transactions.js");
...
function createSuggestion(app, patronRecord, data, options) {
  ...
  let effectiveLibraryOrgId = String(options.effectiveLibraryOrgId || options.libraryOrgId || patronRecord.get("libraryOrgId") || "").trim();
  let effectiveLibraryOrgName = String(options.effectiveLibraryOrgName || options.libraryOrgName || patronRecord.get("libraryOrgName") || "").trim();

  if (!options.skipLimits) {
    duplicates.enforceWeeklyLimit(app, barcode, effectiveLibraryOrgId);
  }
  duplicates.enforceDuplicate(app, barcode, data, effectiveLibraryOrgId);
  ...
  record.set("patronCodeId", patronRecord.get("patronCodeId") || "");
  record.set("patronCodeDescription", patronRecord.get("patronCodeDescription") || "");
  record.set("patronOrgId", patronRecord.get("patronOrgId") || "");
  record.set("libraryOrgId", effectiveLibraryOrgId);
  record.set("libraryOrgName", effectiveLibraryOrgName);
  record.set("preferredPickupBranchId", String(data.preferredPickupBranchId || patronRecord.get("preferredPickupBranchId") || ""));
  record.set("preferredPickupBranchName", String(data.preferredPickupBranchName || patronRecord.get("preferredPickupBranchName") || ""));
  ...
  record.set("customFields", data.customFields && typeof data.customFields === "object" ? data.customFields : {});
  record.set("exactPublicationDate", helpers.normalizeDateOnly(data.exactPublicationDate));
  
  let wf = config.suggestionLimit(app, effectiveLibraryOrgId);
  let autohold = true;
...
function titleRequestToJson(record, app, options) {
  ...
  return {
    ...
    patronCodeId: record.get("patronCodeId") || "",
    patronCodeDescription: record.get("patronCodeDescription") || "",
    title: title,
    author: author,
...
function updateTitleRequest(app, id, data, editedBy) {
  ...
  let fields = ["title", "author", "identifier", "publication", "notes", "bibid", "exactPublicationDate", "autohold", "customFields"];
  for (let i = 0; i < fields.length; i++) {
    if (data[fields[i]] !== undefined && data[fields[i]] !== null) {
      ...
      } else if (fields[i] === "exactPublicationDate") {
...
function deleteTitleRequestWithAudit(app, record, staffUser, mode) {
  return transactions.runInTransaction(app, function (txApp) {
    return deleteTitleRequestWithAuditInApp(txApp, record, staffUser, mode);
  });
}

function deleteTitleRequestWithAuditInApp(app, record, staffUser, mode) {
...
function deleteAdditionalCopyRequestWithAudit(app, record, staffUser, mode) {
  return transactions.runInTransaction(app, function (txApp) {
    return deleteAdditionalCopyRequestWithAuditInApp(txApp, record, staffUser, mode);
  });
}

function deleteAdditionalCopyRequestWithAuditInApp(app, record, staffUser, mode) {
  auditDeletedRequest(app, record, staffUser, mode || "single");
  app.delete(record);
}
...
function deleteClosedRequestsBulk(app, staffUser, confirm) {
  ...
  transactions.runInTransaction(app, function (txApp) {
    suggestionsToDelete.forEach(function (record) {
      deleteTitleRequestWithAuditInApp(txApp, record, staffUser, "bulk");
    });
    
    copiesToDelete.forEach(function (record) {
      deleteAdditionalCopyRequestWithAuditInApp(txApp, record, staffUser, "bulk");
    });
  });
...
module.exports = {
  ...
  deleteTitleRequestWithAuditInApp: deleteTitleRequestWithAuditInApp,
  deleteAdditionalCopyRequestWithAudit: deleteAdditionalCopyRequestWithAudit,
  deleteAdditionalCopyRequestWithAuditInApp: deleteAdditionalCopyRequestWithAuditInApp,
