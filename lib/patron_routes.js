...
function patronLogin(e) {
  ...
  try {
    ...
    patron = polaris.authenticatePatron(barcode, password, staffAuth);
    ...
    var codeEligibility = patronCodes.isEligible(librarySettings.workflow || {}, patron, e.app.logger());
    if (!codeEligibility.allowed) {
      return e.json(403, { message: codeEligibility.message });
    }
    ...
    var pickupContext;
    try {
      pickupContext = pickupPreference.buildAvailablePickupPreferenceContext(e.app, staffAuth, patron);
    } catch (pickupErr) {
      e.app.logger().warn("Patron pickup context load failed during login", "barcode", "[REDACTED]", "error", String(pickupErr));
      pickupContext = {
        pickupBranches: [],
        ...
        pickupBranchWarning: "Pickup locations are temporarily unavailable. You can continue after choosing a location when options load."
      };
    }
...
function createSuggestion(e) {
  ...
  try {
    ...
    var pickupContext = pickupPreference.buildAvailablePickupPreferenceContext(e.app, staffAuth, livePatron);
    var selectedBranch;
    try {
    ...
    if (selectedBranch.id !== liveCurrentPickupId) {
      try {
        ...
        e.app.logger().error("Patron pickup preference update failed", "barcode", "[REDACTED]", "error", String(pickupUpdateErr));
        return e.json(502, {
