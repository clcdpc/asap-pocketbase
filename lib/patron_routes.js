...
function patronLogin(e) {
  ...
  try {
    ...
    var pickupContext;
    try {
      pickupContext = pickupPreference.buildPickupPreferenceContext(e.app, staffAuth, patron);
    } catch (pickupErr) {
...
function createSuggestion(e) {
  ...
  try {
    ...
    var pickupContext = pickupPreference.buildPickupPreferenceContext(e.app, staffAuth, livePatron);
    var selectedBranch;
    try {
      selectedBranch = pickupPreference.validateSelectedPickupBranch(pickupContext, selectedPickupBranchId);
    } catch (pickupErr) {
      pickupContext = pickupPreference.buildPickupPreferenceContext(e.app, staffAuth, livePatron, { forceRefresh: true });
      try {
        selectedBranch = pickupPreference.validateSelectedPickupBranch(pickupContext, selectedPickupBranchId);
      } catch (pickupErr2) {
        return e.json(400, { message: "Choose a valid preferred pickup location." });
