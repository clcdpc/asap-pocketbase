...
function decodeUtf8Bytes(bytes) {
  ...
  if (typeof TextDecoder !== "undefined") {
    try {
      ...
      // Fall through to the small decoder below for the PocketBase hook runtime.
    }
  }
  var str = "";
  try {
...
function effectiveWorkflowRecord(app, orgId) {
  app = app || $app;
  var system = systemWorkflowRecord(app);
  var library = libraryWorkflowRecord(app, orgId);
  if (!library) return system;
  return {
    id: library.id || system.id,
    get: function (key) {
      return hasScopedWorkflowValue(library, key) ? library.get(key) : system.get(key);
    },
...
function additionalCopyTimeout(app, orgId) {
  var wf = suggestionLimit(app, orgId);
  return { enabled: wf.additionalCopyTimeoutEnabled, days: wf.additionalCopyTimeoutDays };
}
