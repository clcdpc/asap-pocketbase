...
function uiTextFromRecord(app, record, orgId) {
  ...
  var brandingOverride = orgId ? libraryBrandingRecord(app, orgId) : null;
  ...
  try {
    // 1. Check for library-specific branding override
    if (brandingOverride) {
      var libraryLogo = brandingOverride.get("logo");
      if (libraryLogo) {
        logoUrl = "/api/files/library_settings/" + brandingOverride.id + "/" + libraryLogo;
        logoAlt = brandingOverride.get("logoAlt");
        brandingSource = "library";
      }
    }

    // 2. Fallback to system default logo if no library override or no logo in override
    if (brandingSource === "system") {
      var systemLogo = record.get("logo");
      if (systemLogo) {
        logoUrl = "/api/files/ui_settings/" + record.id + "/" + systemLogo;
        logoAlt = record.get("logoAlt");
      }
    }
  } catch (err) { }

  if (!logoAlt) logoAlt = "Library Logo";
...
function libraryBrandingRecord(app, orgId) {
  app = app || $app;
  var orgRecordId = orgIdForSettings(app, orgId);
  if (!orgRecordId) return null;
  try {
    return app.findFirstRecordByFilter("library_settings", "libraryOrganization = {:org}", { org: orgRecordId });
  } catch (err) {
    return null;
  }
}
...
module.exports = {
  ...
  libraryBrandingRecord: libraryBrandingRecord,
