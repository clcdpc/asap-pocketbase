...
function evaluatePurchase(app, staff, record, bibCache, result) {
  ...
  if (existingBibId) {
    try {
      var bibInfo = polaris.getBib(app, staff, existingBibId);
      if (bibInfo && (bibInfo.primaryTomId === "36" || bibInfo.primaryTomId === "41")) {
        records.appendSystemNote(record, "Auto-promoter skipped BIB ID " + existingBibId + ": e-materials (" + (bibInfo.format || "electronic") + ") cannot be placed on hold in Polaris.");
        app.save(record);
        return;
      }
    } catch (err) {
      app.logger().warn("Auto-promoter could not verify BIB ID " + existingBibId + "; proceeding with promotion.", "error", String(err));
    }
  ...
  try {
    ...
    if (bibId) {
      ...
      if (bibResult && bibResult.filteredByMaterialType) {
        records.appendSystemNote(record, "Auto-promoter found only e-materials matching identifier " + identifier + "; these cannot be placed on hold in Polaris.");
        app.save(record);
...
function promoteRequestNow(app, staff, record) {
  ...
  if (status === records.STATUS.SUGGESTION) {
    ...
    if (bibResult && bibResult.filteredByMaterialType) {
      target.set("isbnCheckStatus", "not_found");
      records.appendSystemNote(target, "Identifier number verification completed: only e-materials found matching identifier " + identifier + "; these cannot be placed on hold in Polaris.");
      app.save(target);
      return { promoted: false, status: "not_found", reason: "e_material_only" };
    }
