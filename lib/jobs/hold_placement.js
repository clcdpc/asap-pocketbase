...
function processPendingHolds(app, staff, result) {
  ...
  processPagedQueue(app, result, {
    ...
    try {
      ...
      var livePatron = polaris.lookupPatron(staff, barcode);
      var pickupOrgId = String((livePatron && (livePatron.RequestPickupBranchID || livePatron.PatronOrgID)) || "").trim();
      if (!livePatron || !livePatron.PatronID) {
        records.appendSystemNote(record, "SKIP: Patron not found in Polaris using barcode immediately before hold placement.");
        app.save(record);
        result.skipped++;
        return;
      }
      if (records.cachePolarisPatronId) records.cachePolarisPatronId(app, livePatron);
      if (!pickupOrgId) {
        records.addWorkflowTagForRequest(app, record, "Hold failed: pickup");
        records.appendSystemNote(record, "SKIP: Polaris patron record does not include RequestPickupBranchID or PatronOrgID.");
        app.save(record);
        result.skipped++;
        return;
      }

      var hold = polaris.placeHold(staff, bibId, livePatron.PatronID, { pickupOrgId: pickupOrgId, noAutoReply: true });
      ...
      try {
        if (!mail.holdPlaced(app, record, livePatron || patron)) {
