...
function dispatch(app, record, patron, templateKey, defaultSubject, templateId) {
  ...
  const librarySettings = config.librarySettings(app, libraryOrgId);
  const emailsConfig = librarySettings.emails;
  const format = formatLabel(record.get("format"), librarySettings.ui_text && librarySettings.ui_text.formatLabels);
...
function staffReminder(app, record, staff, toEmail, itemUrl, options) {
  ...
  const librarySettings = config.librarySettings(app, record.get("libraryOrgId"));
  const format = formatLabel(record.get("format"), librarySettings.ui_text && librarySettings.ui_text.formatLabels);
