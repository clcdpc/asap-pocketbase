...
const embedSecurity = require("../embed_security.js");
...
function saveSystemSettings(app, data) {
  ...
  if (data && Object.prototype.hasOwnProperty.call(data, "leapPatronUrlPattern")) {
    record.set("leapPatronUrlPattern", data.leapPatronUrlPattern);
  }
  ...
  if (data && Object.prototype.hasOwnProperty.call(data, "patronEmbedAllowedOrigins")) {
    record.set("patronEmbedAllowedOrigins", embedSecurity.normalizeEmbedAllowedOrigins(data.patronEmbedAllowedOrigins));
  }
  app.save(record);
  return record;
}
...
function getSettings(app) {
  ...
  return Object.assign({
    ...
    leapPatronUrlPattern: sys ? sys.get("leapPatronUrlPattern") || "" : "",
    formatIconUrlPattern: normalizeFormatIconUrlPattern(
      sys ? sys.get("formatIconUrlPattern") || "" : ""
    ),
    patronEmbedAllowedOrigins: sys ? sys.get("patronEmbedAllowedOrigins") || "" : "",
    patronEmbedFrameAncestors: embedSecurity.frameAncestorsCsp(sys ? sys.get("patronEmbedAllowedOrigins") || "" : ""),
...
function librarySettings(app, libraryOrgId) {
  ...
  return {
    ...
    leapPatronUrlPattern: sys.get("leapPatronUrlPattern") || "",
    formatIconUrlPattern: normalizeFormatIconUrlPattern(
      sys.get("formatIconUrlPattern") || ""
    ),
    patronEmbedAllowedOrigins: sys.get("patronEmbedAllowedOrigins") || "",
    patronEmbedFrameAncestors: embedSecurity.frameAncestorsCsp(sys.get("patronEmbedAllowedOrigins") || "")
  };
}
...
function patronEmbedFrameAncestors(app, requestAncestorUrl) {
  var sys = getSystemSettings(app || $app);
  return embedSecurity.frameAncestorsCspForRequest(sys ? sys.get("patronEmbedAllowedOrigins") || "" : "", requestAncestorUrl || "");
}
...
module.exports = {
  ...
  patronEmbedFrameAncestors: patronEmbedFrameAncestors,
};
