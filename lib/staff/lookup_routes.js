...
function staffPatronLookupResponse(app, staffAuth, patronRecord, patronData, meta) {
  var pickupContext = pickupPreference.buildAvailablePickupPreferenceContext(app, staffAuth, patronData);
  return withScopeMeta({
    ...
    preferredPickupBranchId: pickupContext.currentPreferredPickupBranchId || patronData.PreferredPickupBranchID || "",
    preferredPickupBranchName: pickupContext.currentPreferredPickupBranchName || patronData.PreferredPickupBranchName || "",
...
function resolveStaffPatronByBarcode(e, staff, staffAuth, barcode, context, allowAnyRegisteredCardLogin, meta) {
  ...
  return staffPatronLookupResponse(e.app, staffAuth, patronRecord, patronData, meta);
}
...
function staffLookupPatron(e) {
  try {
    ...
    if (search.status === "error") {
      var safeRaw = raw.replace(/[\r\n]+/g, " ");
      e.app.logger().warn("Staff patron name search failed", "query", safeRaw, "error", search.error || "");
    ...
    if (barcode && info) {
      ...
      try {
        ...
        if (patron && patron.PatronID) {
          ...
          if (records.cachePolarisPatronId) records.cachePolarisPatronId(e.app, patron);
