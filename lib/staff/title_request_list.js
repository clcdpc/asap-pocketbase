...
function findAllRecordsByFilter(app, collection, filter, sort, limit, params) {
  var out = [];
  var offset = 0;

  while (true) {
    var rows = app.findRecordsByFilter(collection, filter, sort || "", limit, offset, params);
...
function preloadWorkflowTagsForRequests(app, titleRequests, workflowTagsCache) {
  ...
  for (var b = 0; b < requestIds.length; b += batchSize) {
    ...
    var joinRows = findAllRecordsByFilter(
      app,
      "title_request_tags",
      filterParts.join(" || "),
      "",
      Math.max(batchIds.length * 10, 100),
      batchParams
    );
    if (joinRows && joinRows.length > 0) {
      for (var jr = 0; jr < joinRows.length; jr++) {
        allJoinRows.push(joinRows[jr]);
      }
    }
  }

  if (allJoinRows.length === 0) {
    return;
  }
  ...
  for (var k = 0; k < allJoinRows.length; k++) {
    var reqId = allJoinRows[k].get("titleRequest");
    var tId = allJoinRows[k].get("tag");
    if (reqId && tId) {
      if (!joinMap[tId]) {
        joinMap[tId] = [];
...
function enrichRowWithPatron(row, patronRecord) {
  ...
  row.polarisPatronId = patronRecord ? patronRecord.get("polarisPatronId") || "" : "";
}
