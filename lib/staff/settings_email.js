...
function saveRejectionTemplatesInApp(app, scope, orgId, templates) {
  ...
  if (templateIds.length > 0) {
    var filterParts = [];
    var batchParams = {};
    for (var m = 0; m < templateIds.length; m++) {
      filterParts.push("id = {:p" + m + "}");
      batchParams["p" + m] = templateIds[m];
    }
    var batchFilter = filterParts.join(" || ");
    try {
      var results = app.findRecordsByFilter("rejection_templates", batchFilter, "", templateIds.length, 0, batchParams);
