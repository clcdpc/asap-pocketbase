...
function saveRejectionTemplatesInApp(app, scope, orgId, templates) {
  ...
  var hiddenSourceIds = {};
  var systemTemplates = {};
  var libraryRecordsBySourceId = {};
  var libraryRows = [];

  function sameTemplateContent(row, template) {
    return String(row.get("name") || "") === String(template.name || "") &&
      String(row.get("subject") || "") === String(template.subject || "") &&
      String(row.get("body") || "") === String(template.body || "");
  }

  if (scope === "library") {
    try {
      var systemRows = app.findRecordsByFilter("rejection_templates", "scope = 'system' && enabled = true", "sortOrder", 200, 0, {});
      systemRows.forEach(function (row) {
        systemTemplates[row.id] = row;
      });
    } catch (errSystem) {}

    try {
      libraryRows = app.findRecordsByFilter("rejection_templates", "scope = 'library' && libraryOrganization = {:org}", "sortOrder", 200, 0, { org: org.id });
      libraryRows.forEach(function (row) {
        var sourceTemplateId = String(row.get("sourceTemplateId") || "").trim();
        if (sourceTemplateId) libraryRecordsBySourceId[sourceTemplateId] = row;
      });
    } catch (errLibrary) {}
  }
    ...
    for (var k = 0; k < templateIds.length; k += batchSize) {
      ...
      try {
        ...
        for (var n = 0; n < results.length; n++) {
          if (scope !== "library" || (results[n].get("scope") === "library" && results[n].get("libraryOrganization") === org.id)) {
  ...
  var rejectionTemplatesCollection = app.findCollectionByNameOrId("rejection_templates");
  ...
  for (var i = 0; i < templates.length; i++) {
    ...
    if (scope === "library") {
      sourceTemplateId = String(t.sourceTemplateId || "").trim();
      if (!sourceTemplateId && t.id && systemTemplates[String(t.id)]) {
        sourceTemplateId = String(t.id);
      }
      if (sourceTemplateId) {
        hiddenSourceIds[sourceTemplateId] = false;
        if (systemTemplates[sourceTemplateId] && sameTemplateContent(systemTemplates[sourceTemplateId], t)) {
          if (libraryRecordsBySourceId[sourceTemplateId]) app.delete(libraryRecordsBySourceId[sourceTemplateId]);
          continue;
        }
        record = libraryRecordsBySourceId[sourceTemplateId] || null;
      }
    }
    ...
    if (!record) {
      isNew = true;
      record = new Record(rejectionTemplatesCollection);
      record.set("scope", scope);
      if (org) record.set("libraryOrganization", org.id);
    }

    var oldSourceTemplateId = record.get("sourceTemplateId") || "";
    var oldName = record.get("name") || "";
    var oldSubject = record.get("subject") || "";
    var oldBody = record.get("body") || "";
    var oldEnabled = record.get("enabled");
    var oldSortOrder = record.get("sortOrder");

    var newSourceTemplateId = sourceTemplateId || oldSourceTemplateId;
    var newName = t.name || "Rejection template";
    var newSubject = t.subject || "";
    var newBody = t.body || "";
    var newEnabled = true;
    var newSortOrder = i + 1;

    if (sourceTemplateId) record.set("sourceTemplateId", sourceTemplateId);
    record.set("name", newName);
    record.set("subject", newSubject);
    record.set("body", newBody);
    record.set("enabled", newEnabled);
    record.set("sortOrder", newSortOrder);

    var changed = isNew ||
      String(oldSourceTemplateId) !== String(newSourceTemplateId) ||
      String(oldName) !== String(newName) ||
      String(oldSubject) !== String(newSubject) ||
      String(oldBody) !== String(newBody) ||
      Boolean(oldEnabled) !== Boolean(newEnabled) ||
      Number(oldSortOrder) !== Number(newSortOrder);
  ...
  if (scope === "library") {
    Object.keys(systemTemplates).forEach(function (sourceTemplateId) {
      var present = false;
      for (var presentIndex = 0; presentIndex < templates.length; presentIndex++) {
        var presentTemplate = templates[presentIndex] || {};
        var presentSourceId = String(presentTemplate.sourceTemplateId || "").trim();
        if (!presentSourceId && presentTemplate.id && String(presentTemplate.id) === sourceTemplateId) presentSourceId = sourceTemplateId;
        if (presentSourceId === sourceTemplateId) {
          present = true;
          break;
        }
      }
      if (present) return;
      hiddenSourceIds[sourceTemplateId] = true;
    });

    var toHide = [];
    Object.keys(hiddenSourceIds).forEach(function (sourceTemplateId) {
      if (!hiddenSourceIds[sourceTemplateId]) return;
      toHide.push(sourceTemplateId);
    });

    if (toHide.length > 0) {
      var hideCheckFilter = [];
      var hideCheckParams = {};
      toHide.forEach(function (sourceTemplateId, index) {
        var p = "hide" + index;
        hideCheckFilter.push("outstandingTimeoutRejectionTemplate = {:" + p + "}");
        hideCheckParams[p] = sourceTemplateId;
      });
      var hiddenInUseRecords = app.findRecordsByFilter("workflow_settings", hideCheckFilter.join(" || "), "", 1, 0, hideCheckParams);
      if (hiddenInUseRecords && hiddenInUseRecords.length > 0) {
        var hiddenInUseErr = new Error(TEMPLATE_IN_USE_BY_AUTO_REJECT_MESSAGE);
        hiddenInUseErr.code = TEMPLATE_IN_USE_BY_AUTO_REJECT_CODE;
        throw hiddenInUseErr;
      }

      toHide.forEach(function (sourceTemplateId) {
        var hiddenRecord = libraryRecordsBySourceId[sourceTemplateId] || new Record(app.findCollectionByNameOrId("rejection_templates"));
        hiddenRecord.set("scope", "library");
        hiddenRecord.set("libraryOrganization", org.id);
        hiddenRecord.set("sourceTemplateId", sourceTemplateId);
        hiddenRecord.set("name", systemTemplates[sourceTemplateId].get("name") || "Rejection template");
        hiddenRecord.set("subject", systemTemplates[sourceTemplateId].get("subject") || "");
        hiddenRecord.set("body", systemTemplates[sourceTemplateId].get("body") || "");
        hiddenRecord.set("enabled", false);
        hiddenRecord.set("sortOrder", systemTemplates[sourceTemplateId].get("sortOrder") || 0);
        app.save(hiddenRecord);
        if (hiddenRecord.id) keep[String(hiddenRecord.id)] = true;
      });
    }
  }
