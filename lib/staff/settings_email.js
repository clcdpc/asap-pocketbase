...
function saveEmailSettings(app, scope, orgId, emails) {
  return transactions.runInTransaction(app, function (txApp) {
    ...
    var params = scope === "system" ? {} : { org: orgId || "" };

    var keyFilters = [];
    EMAIL_TEMPLATE_KEYS.forEach(function(k, i) {
        keyFilters.push("templateKey = {:k" + i + "}");
        params["k" + i] = k;
    });
    if (keyFilters.length > 0) {
        filter = filter + " && (" + keyFilters.join(" || ") + ")";
    }

    try {
        var rows = txApp.findRecordsByFilter("email_templates", filter, "", EMAIL_TEMPLATE_KEYS.length, 0, params);
        rows.forEach(function(row) {
            existingTemplates[row.get("templateKey")] = row;
        });
    } catch (err) {
        // Ignored
    }
    ...
    EMAIL_TEMPLATE_KEYS.forEach(function (key) {
      ...
      if (!tpl.subject && !tpl.body && scope === "library") {
        var existing = existingTemplates[key];
        if (existing) toDelete.push(existing);
        return;
      }
      var record = existingTemplates[key];
      if (!record) {
        record = new Record(txApp.findCollectionByNameOrId("email_templates"));
        record.set("scope", scope);
        if (scope === "library" && orgId) record.set("libraryOrganization", orgId);
      }
