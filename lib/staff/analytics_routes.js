...
const titleRequestList = require(`${__hooks}/../lib/staff/title_request_list.js`);
...
function resolveAnalyticsDateRange(key, now) {
  ...
  var cleanKey = key === "thisMonth" || key === "lastMonth" || key === "last90" ? key : "last30";
  var start, end;

  if (cleanKey === "thisMonth") {
    start = new Date(nowDate.getFullYear(), nowDate.getMonth(), 1, 0, 0, 0, 0);
    end = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate(), 23, 59, 59, 999);
  } else if (cleanKey === "lastMonth") {
    start = new Date(nowDate.getFullYear(), nowDate.getMonth() - 1, 1, 0, 0, 0, 0);
    end = new Date(nowDate.getFullYear(), nowDate.getMonth(), 0, 23, 59, 59, 999);
  } else {
    var days = cleanKey === "last90" ? 90 : 30;
    start = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate() - days, 0, 0, 0, 0);
    end = new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate(), 23, 59, 59, 999);
  }
...
function loadExceptionCounts(app, scope, dateRange, rows) {
  ...
  titleRequestList.preloadWorkflowTagsForRequests(app, rows, workflowTagsCache);
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var tags = workflowTagsCache[row.id] || [];
