  ...
  try {
    var result = claimUtils.runClaimMutation(e.app, function (app) {
      ...
      if (!routeUtils.sameLibrary(actor, task.get("libraryOrgId"))) {
        var accessErr = new Error("Additional-copy request not found.");
        accessErr.statusCode = 404;
        throw accessErr;
      }
      ...
      try {
        ...
        e.app.logger().error("Assignment policy assertion failed", "requestId", id, "actorId", actor.id, "assigneeId", assigneeId, "error", String(policyErr));
        throw policyErr;
      }
...
module.exports = {
  ...
  staffAssignAdditionalCopy: staffAssignAdditionalCopy
};
