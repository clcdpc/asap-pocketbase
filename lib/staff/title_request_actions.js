...
function buildPickupContextForEditableRequest(app, staffAuth, livePatron, options) {
  ...
  var pickupContext = pickupPreference.buildPickupPreferenceContext(app, staffAuth, livePatron, options);

  if (!options.forceRefresh && !(pickupContext.pickupBranches || []).length) {
    var forceOptions = Object.assign({}, options, { forceRefresh: true });
    pickupContext = pickupPreference.buildPickupPreferenceContext(app, staffAuth, livePatron, forceOptions);
  }
