...
function validateTitleRequestAction(context, e) {
  ...
  var currentStatus = context.oldStatus || (records.normalizeStatus ? records.normalizeStatus(context.record.get("status")) : String(context.record.get("status") || ""));
  if (records.STATUS.HOLD_PLACED && currentStatus === records.STATUS.HOLD_PLACED && context.data.bibid !== undefined && context.data.bibid !== null) {
    var existingBibId = String(context.record.get("bibid") || "").trim();
    var nextBibId = String(context.data.bibid || "").trim();
    if (nextBibId !== existingBibId) {
      return e.json(400, { message: "BIB ID cannot be changed after the hold has been placed." });
    }
  }
  return null;
}
...
function persistTitleRequestAction(e, context) {
  ...
  sanitizeCustomFieldsForAction(e, context);
...
function sanitizeCustomFieldsForAction(e, context) {
  if (context.data.customFields === undefined) return;
  var uiText = config.uiText(e.app, context.record.get("libraryOrgId"));
  var format = String(context.data.format || context.record.get("format") || "book").trim() || "book";
  var formatRule = (uiText.formatRules && uiText.formatRules[format]) || {};
  var rules = formatRule.customFields || {};
  var definitions = uiText.additionalFieldDefinitions || [];
  var submitted = context.data.customFields || {};
  var sanitized = customFields.sanitizeSubmittedValues(submitted, definitions, rules);
  var existing = parseObject(context.record.get("customFields"));
  var merged = Object.assign({}, existing);
  var normalizedRules = customFields.normalizeFormatCustomFieldRules(rules, definitions);

  definitions.forEach(function (def) {
    if (!def || !def.key) return;
    var rule = normalizedRules[def.key] || { mode: "hidden" };
    if (rule.mode === "hidden") return;
    if (Object.prototype.hasOwnProperty.call(sanitized, def.key)) {
      merged[def.key] = sanitized[def.key];
    } else {
      delete merged[def.key];
    }
  });

  // Preserve submitted values for fields not in the current library's definitions.
  // This handles the super-admin case where the frontend renders fields using
  // one library's definitions but the request belongs to another library.
  Object.keys(submitted).forEach(function (key) {
    if (Object.prototype.hasOwnProperty.call(merged, key)) return;
    var val = submitted[key];
    if (val && typeof val === "object") {
      merged[key] = val;
    } else if (typeof val === "string" && val.trim()) {
      merged[key] = { value: val.trim() };
    }
  });

  context.data.customFields = merged;
}
...
function staffTitleRequestPickupOptions(e) {
  try {
    ...
    if (records.cachePolarisPatronId) records.cachePolarisPatronId(e.app, livePatron);
