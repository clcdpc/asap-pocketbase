...
function staffCreateSuggestion(e) {
  ...
  var pickupContext = pickupPreference.buildAvailablePickupPreferenceContext(e.app, staffAuth, patronData);
  var selectedBranch;
  try {
    selectedBranch = pickupPreference.validateSelectedPickupBranch(pickupContext, selectedPickupBranchId);
  } catch (pickupErr) {
    pickupContext = pickupPreference.buildAvailablePickupPreferenceContext(e.app, staffAuth, patronData, { forceRefresh: true });
    try {
      selectedBranch = pickupPreference.validateSelectedPickupBranch(pickupContext, selectedPickupBranchId);
    } catch (pickupErr2) {
      return e.json(400, { message: "Choose a valid preferred pickup location." });
  ...
  if (pickupChanged) {
    try {
      ...
      e.app.logger().error("Staff pickup preference update failed", "barcode", "[REDACTED]", "staff", staff.get("username") || "", "error", String(pickupUpdateErr));
      return e.json(502, {
  ...
  try {
    ...
    // Send confirmation email before identifier lookup so it uses the patron's
    // original entered title/author rather than Polaris-resolved values.
...
function staffSyncOrganizations(e) {
  ...
  try {
    ...
    var result = orgs.syncOrganizations(e.app, staffAuth);
    var patronCodeResult = { synced: 0 };
    var patronCodesError = "";
    try {
      patronCodeResult = patronCodes.syncPatronCodes(e.app, staffAuth);
    } catch (patronCodeErr) {
      patronCodesError = patronCodeErr && patronCodeErr.message ? patronCodeErr.message : String(patronCodeErr);
      e.app.logger().warn("Polaris patron code sync failed during manual reference data sync", "error", patronCodesError);
    }
    return e.json(200, {
      success: true,
      synced: result.synced || 0,
      patronCodesSynced: patronCodeResult.synced || 0,
      patronCodesError: patronCodesError,
      message: patronCodesError
        ? "Organization hierarchy synced, but patron codes could not be loaded."
        : "Organization hierarchy and patron codes synced."
    });
  } catch (err) {
    return e.json(400, { success: false, message: err.message || String(err) });
  }
...
function staffSyncPatronCodes(e) {
  if (!routeUtils.requireSuperAdminStaff(e)) {
    return e.json(403, { message: "Super admin access required" });
  }
  try {
    var result = patronCodes.syncPatronCodes(e.app);
    return e.json(200, {
      success: true,
      synced: result.synced || 0,
      message: "Patron codes synced."
    });
  } catch (err) {
    e.app.logger().error("Polaris patron code sync failed", "error", err && err.message ? err.message : String(err));
    return e.json(400, { success: false, message: err.message || String(err) });
  }
}
