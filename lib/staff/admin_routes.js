...
function effectiveStaffLibraryContext(e, staff, data) {
  data = data || {};
...
function staffCreateSuggestion(e) {
  ...
  var effectiveLibrary = effectiveStaffLibraryContext(e, staff, data);
  ...
  try {
    ...
    var wf = config.workflowSettings ? config.workflowSettings(e.app, effectiveLibrary.libraryOrgId) : {};
    if (!wf.allowAnyRegisteredCardLogin && String(patronData.LibraryOrgID || "").trim() !== String(effectiveLibrary.libraryOrgId || "").trim()) {
  ...
  var patronRecord = records.upsertPatronUser(e.app, patronData, {
    experienceLibraryOrgId: effectiveLibrary.libraryOrgId,
    experienceLibraryOrgName: effectiveLibrary.libraryOrgName,
    effectiveLibraryOrgId: effectiveLibrary.libraryOrgId,
    effectiveLibraryOrgName: effectiveLibrary.libraryOrgName
  });

  try {
    data.staffLibraryOrgIdCreatedBy = effectiveLibrary.libraryOrgId || staff.get("libraryOrgId") || "";
