...
function staffUsersList(e) {
  try {
    ...
    var users = allStaff.filter(function (record) {
      try {
        if (isSuper) {
          return !targetOrgId || targetOrgId === "system" || targetOrgId === "all" || routeUtils.sameLibrary(record, targetOrgId);
        }
        return routeUtils.sameLibrary(admin, record.get("libraryOrgId"));
      } catch (filterErr) {
        return false;
      }
    });
