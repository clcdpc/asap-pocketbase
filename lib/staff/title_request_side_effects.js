...
function purchaseReminderSuppressedByFinalStatus(context) {
  var status = finalStatus(context);
  return actionStartedAsPurchase(context) && status !== records.STATUS.OUTSTANDING_PURCHASE;
}

function handleStaffActionPatronEmailSideEffects(app, context) {
  ...
  if (context.action === "purchase") {
    handlePurchaseOutcomeSideEffects(app, context);
  }
}

function handleAlreadyOwnOrRejectSideEffects(app, context) {
  handleStaffActionPatronEmailSideEffects(app, context);
}

function handlePurchaseOutcomeSideEffects(app, context) {
  var status = finalStatus(context);

  if (status === records.STATUS.OUTSTANDING_PURCHASE && context.oldStatus !== records.STATUS.OUTSTANDING_PURCHASE) {
    sendPurchaseApprovedActionEmail(app, context.record, refreshedActionPatron(app, context.record));
    return;
  }
  ...
  if (status === records.STATUS.HOLD_PLACED) {
    sendAlreadyOwnedActionEmail(app, context.record, refreshedActionPatron(app, context.record));
  }
}

function refreshedActionPatron(app, record) {
  try {
    var patron = polaris.lookupPatron(polaris.adminStaffAuth(), record.get("barcode"));
    if (records.cachePolarisPatronId) records.cachePolarisPatronId(app, patron);
    return patron;
  } catch (err) {
...
function handleAlreadyOwnSideEffects(app, context, patron) {
  ...
  var patronId = String((patron && patron.PatronID) || "").trim();
  if (!patronId && records.cachedPolarisPatronIdForTitleRequest) {
    patronId = records.cachedPolarisPatronIdForTitleRequest(app, context.record);
  }
  if (bibid && patronId) {
    placeAlreadyOwnedHold(app, context.record, bibid, patronId);
  }
  sendAlreadyOwnedActionEmail(app, context.record, patron);
}

function placeAlreadyOwnedHold(app, record, bibid, patronId) {
  var localStaffAuth;
  try {
  ...
  try {
    polaris.placeHold(localStaffAuth, bibid, patronId, false);
...
function sendPurchaseApprovedActionEmail(app, record, patron) {
  try {
    if (!mail.purchaseApproved(app, record, patron)) {
      routeUtils.noteSkippedEmail(app, record);
    }
  } catch (mailErr) {
    app.logger().error("Purchase-approved email failed", "recordId", record.id, "error", String(mailErr));
  }
}
...
function sendPurchaseReminderIfRequested(app, context) {
  ...
  var isPurchaseReminderEligible = actionStartedAsPurchase(context) && finalStatus(context) === records.STATUS.OUTSTANDING_PURCHASE;
  ...
  if (purchaseReminderSuppressedByFinalStatus(context) && context.data.emailPurchaseReminder === true) {
    purchaseReminderEmail.requested = true;
    purchaseReminderEmail.sent = false;
...
module.exports = {
  ...
  purchaseReminderSuppressedByFinalStatus,
  handleStaffActionPatronEmailSideEffects,
  ...
  sendPurchaseApprovedActionEmail,
