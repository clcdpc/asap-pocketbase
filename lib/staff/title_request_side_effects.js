...
function sendPurchaseReminderIfRequested(app, context) {
  ...
  var purchaseReminderEligible = context.action === "purchase" && finalStatus(context) === records.STATUS.OUTSTANDING_PURCHASE;
  var purchaseReminderEmail = {
    requested: (purchaseReminderEligible || isAdditionalCopy) && context.data.emailPurchaseReminder === true,
    sent: false,
    message: ""
  };

  if (context.action === "purchase" && context.data.emailPurchaseReminder === true && !purchaseReminderEligible) {
