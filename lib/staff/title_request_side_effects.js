...
function handleAlreadyOwnOrRejectSideEffects(app, context) {
  if (context.action === "alreadyOwn") {
    var alreadyOwnPatron = refreshedActionPatron(app, context.record);
    handleAlreadyOwnSideEffects(app, context, alreadyOwnPatron);
    return;
  }

  if (context.action === "reject") {
    var rejectPatron = refreshedActionPatron(app, context.record);
    sendRejectedActionEmail(app, context, rejectPatron);
    return;
  }
...
function sendPurchaseReminderIfRequested(app, context) {
  ...
  if (context.action === "purchase" && context.data.emailPurchaseReminder === true && !isPurchaseReminderEligible) {
