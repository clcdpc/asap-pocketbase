  ...
  try {
    ...
    if (orgId === "system") {
      ...
      return e.json(200, {
        ...
        leapPatronUrlPattern: s.leapPatronUrlPattern || "",
        formatIconUrlPattern: s.formatIconUrlPattern || "",
        patronEmbedAllowedOrigins: s.patronEmbedAllowedOrigins || "",
        patronEmbedFrameAncestors: s.patronEmbedFrameAncestors || "",
        emailStatus: config.emailStatus(e.app, ""),
        organizationSync: organizationSyncStatus(e.app),
        patronCodesSync: patronCodesSyncStatus(e.app),
        isOverride: false
      });
    }
    ...
    return e.json(200, {
      ...
      leapPatronUrlPattern: ls.leapPatronUrlPattern || "",
      formatIconUrlPattern: ls.formatIconUrlPattern || "",
      patronEmbedAllowedOrigins: "",
      patronEmbedFrameAncestors: ls.patronEmbedFrameAncestors || "",
...
function patronCodesSyncStatus(app) {
  var sys = config.getSystemSettings(app);
  return {
    status: sys ? sys.get("patronCodesSyncStatus") || "not_loaded" : "not_loaded",
    message: sys ? sys.get("patronCodesSyncMessage") || "" : "",
    error: sys ? sys.get("patronCodesSyncError") || "" : "",
    lastSynced: sys ? sys.get("patronCodesLastSynced") || "" : ""
  };
}
...
function updateLibrarySettings(e) {
  ...
  if (orgId !== "system" && !routeUtils.isSuperAdmin(staff)) {
    ...
    delete payload.leapPatronUrlPattern;
    delete payload.patronEmbedAllowedOrigins;
    if (payload.workflow) {
