...
function staffAssignTitleRequest(e) {
  ...
  try {
    var result = runClaimMutation(e.app, function (app) {
      ...
      records.recordEvent(
        ...
        "Claim transferred to " + staffClaimDisplayName(assignee) + " by " + staffClaimDisplayName(actor) + ".",
        {
          actorName: actor.get("username") || staffClaimDisplayName(actor)
        }
      );
    ...
    sendAssignmentNotificationAfterCommit(e.app, result);
