...
function saveEnabledLibraries(app, csv) {
  ...
  var idArray = String(csv || "").split(",").map(function (s) { return s.trim(); }).filter(Boolean);
  var ids = new Set(idArray);
  ...
  transactions.runInTransaction(app, function (txApp) {
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      var enabled = idArray.length === 0 || ids.has(organizationId);

      if (all[i].get("enabledForPatrons") !== enabled) {
        all[i].set("enabledForPatrons", enabled);
        txApp.save(all[i]);
      }

      if (ids.has(organizationId)) rels.push(all[i].id);
    }
