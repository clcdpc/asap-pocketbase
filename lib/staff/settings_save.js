...
function saveSystemSettingsPayload(app, payload) {
  ...
  if (payload.polaris) {
    ...
    if (polarisData.host && polarisData.accessId && polarisData.apiKey) {
      ...
      var orgSyncOk = false;
      try {
        staffAuth = polaris.adminStaffAuth(polarisData);
      ...
      if (staffAuth && orgSyncOk) {
        try {
          patronCodes.syncPatronCodes(app, staffAuth, polarisData);
        } catch (patronCodeSyncErr) {
          app.logger().warn("Polaris patron code sync failed after settings save", "error", String(patronCodeSyncErr));
        }
      }
    }
  }
...
function saveEnabledLibraries(app, csv) {
  ...
  var ids = splitList.split(String(csv || ""));
  ...
  transactions.runInTransaction(app, function (txApp) {
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      if (all[i].getBool("enabledForPatrons") !== enabled) {
...
function saveWorkflowSettings(app, scope, orgId, wf) {
  ...
  ["suggestionLimit", "suggestionLimitMessage", "outstandingTimeoutEnabled", "outstandingTimeoutDays", "outstandingTimeoutSendEmail", "holdPickupTimeoutEnabled", "holdPickupTimeoutDays", "pendingHoldTimeoutEnabled", "pendingHoldTimeoutDays", "additionalCopyTimeoutEnabled", "additionalCopyTimeoutDays", "autoPromote", "commonAuthorsEnabled", "commonAuthorsList", "commonAuthorsMessage", "commonAuthorsLabel", "commonAuthorsHelp", "allowPatronAutoholdOptOut", "allowAnyRegisteredCardLogin", "patronCodeEligibilityEnabled", "allowedPatronCodeIds", "patronCodeEligibilityMessage", "externalSearch1Enabled", "externalSearch1Label", "externalSearch1UrlTemplate", "externalSearch2Enabled", "externalSearch2Label", "externalSearch2UrlTemplate", "externalSearch3Enabled", "externalSearch3Label", "externalSearch3UrlTemplate", "externalSearch4Enabled", "externalSearch4Label", "externalSearch4UrlTemplate"].forEach(function (key) {
