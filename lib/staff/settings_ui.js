  ...
  try {
    ...
    if (toCheck.length > 0) {
      ...
      for (var k = 0; k < toCheck.length; k++) {
        codeValues.push(toCheck[k].get("code"));
      }
      try {
        var usedRequest = app.findFirstRecordByFilter("title_requests", "format ?= {:formats}", { formats: codeValues });
        if (usedRequest) {
          var usedCode = usedRequest.get("format");
          var usedLabel = "";
          for (var k = 0; k < toCheck.length; k++) {
            if (toCheck[k].get("code") === usedCode) {
              usedLabel = toCheck[k].get("label") || usedCode;
              break;
            }
          }
