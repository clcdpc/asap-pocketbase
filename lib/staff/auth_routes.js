...
function staffLogin(e) {
  try {
    ...
    if (e.app && e.app.logger) {
      if (err instanceof UnauthorizedError || (err.message && err.message.indexOf('Invalid credentials') >= 0)) {
        e.app.logger().warn("Staff login failed: Invalid credentials", "username", data && data.username);
      } else {
        e.app.logger().error("Staff login failed", "error", String(err));
      }
    }
    return e.json(401, { message: "Invalid credentials or login failed. Please try again." });
  }
}
