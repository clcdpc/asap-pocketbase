...
function upsertPatronUser(app, patron) {
  ...
  let record = findFirstByData(app, "patron_users", "barcode", barcode);

  if (!record && email) {
    // If not found by barcode, try finding by email AND name to handle barcode changes for the same person
    let existingByEmail = findFirstByData(app, "patron_users", "email", email);
    if (!existingByEmail) {
...
function removeWorkflowTagForRequest(app, record, tag) {
  ...
  let tagRecord = lookupByCode(app, "workflow_tags", cleanTag);
...
function flagCrossPatronDuplicateSuggestion(app, record) {
  ...
  try {
    ...
    let added = addWorkflowTagForRequest(app, record, WORKFLOW_TAG_DUPLICATE_SUGGESTION);
...
function deleteAdditionalCopyRequestWithAudit(app, record, staffUser, mode) {
  auditDeletedRequest(app, record, staffUser, mode || "single");
  // additional_copy_requests don't have related tags/events in separate collections yet.
  app.delete(record);
}
