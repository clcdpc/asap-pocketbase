...
function upsertPatronUser(app, patron) {
  ...
  var email = safeEmail(patron.EmailAddress);

  var record = findFirstByData(app, "patron_users", "barcode", barcode);

  if (!record && email) {
    // If not found by barcode, try finding by email AND name to handle barcode changes for the same person
    var existingByEmail = findFirstByData(app, "patron_users", "email", email);
    if (!existingByEmail) {
        // Also check our new notificationEmail field
        try {
    ...
    if (existingByEmail) {
      var existingFirst = String(existingByEmail.get("nameFirst") || "").toLowerCase().trim();
      var existingLast = String(existingByEmail.get("nameLast") || "").toLowerCase().trim();
      var newFirst = String(patron.NameFirst || "").toLowerCase().trim();
      var newLast = String(patron.NameLast || "").toLowerCase().trim();
  ...
  // Always set a unique auth email based on barcode to bypass PB uniqueness constraints on the real email
  ...
  // Store the real email in a non-unique field
  if (email) {
...
function createSuggestion(app, patronRecord, data, options) {
  options = options || {};
  var barcode = String(patronRecord.get("barcode") || "").trim();
  ...
  // Use real email from options, then from patron record's notification field, then fallback to auth email
  var patronEmail = options.email || (typeof patronRecord.get === "function" ? patronRecord.get("notificationEmail") : "") || patronRecord.email();
  ...
  // Enforce title/author length again at persistence layer
  record.set("title", titleCase(String(data.title || "").slice(0, 500)));
  record.set("author", String(data.author || "").trim().slice(0, 500));
  
  var identifier = String(data.identifier || data.isbn || "").trim().slice(0, 100);
  ...
  var wf = config.suggestionLimit(app, patronRecord.get("libraryOrgId"));
  var autohold = true;
...
function polarisSubmittedSearchValue(value) {
  ...
  var wrapped = text.match(/^(.*)\(([^()]*)\)\s*$/);
  if (wrapped) {
    var prefix = String(wrapped[1] || "").trim();
    var original = String(wrapped[2] || "").trim();
...
function titleRequestToJson(record, app, options) {
  ...
  return {
    ...
    title: title,
    author: author,
    polarisSearchTitle: polarisSubmittedSearchValue(title),
    polarisSearchAuthor: polarisSubmittedSearchValue(author),
    ...
    workflowTags: options.workflowTags !== undefined ? options.workflowTags : (app ? workflowTagsForRequest(app, record) : []),
...
function removeWorkflowTagForRequest(app, record, tag) {
  ...
  var removed = false;
  try {
    var rows = app.findRecordsByFilter("title_request_tags", "titleRequest = {:request} && tag = {:tag}", "", 100, 0, { request: record.id, tag: tagRecord.id });
...
function addWorkflowTagForRequest(app, record, tag) {
  ...
  var conflicts = conflictingWorkflowTagNames(cleanTag);
...
function updateTitleRequest(app, id, data, editedBy) {
  ...
  var fields = ["title", "author", "identifier", "publication", "notes", "bibid", "exactPublicationDate", "autohold"];
