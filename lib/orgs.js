...
function syncOrganizations(app, staffAuth) {
  ...
  try {
    ...
    for (var i = 0; i < kinds.length; i++) {
      rowsByKind.push(polaris.organizations(kinds[i], staffAuth));
    }
    transactions.runInTransaction(app, function (txApp) {
      for (var i = 0; i < rowsByKind.length; i++) {
        var rows = rowsByKind[i];
        for (var j = 0; j < rows.length; j++) {
          if (upsertOrganization(txApp, rows[j])) {
            count++;
          }
        }
      }
      relinkParents(txApp);
      setSyncStatus(txApp, "loaded", "Polaris organizations loaded successfully.", "");
    });
    return { synced: count };
  } catch (err) {
