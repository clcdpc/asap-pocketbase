...
function normalizeNumeric(value) {
  ...
  if (!str) return 0;
  // Handle comma-separated strings that might come from bad byte-array conversions
  if (str.indexOf(",") !== -1) {
    str = str.split(",")[0];
  }
...
function getMARCTypeOfMaterials(staff) {
  var ep = endpoint("public", "marctypeofmaterials");
  var payload = send("GET", ep, "", staff || null);
  var rows = normalizeRows(payload.MARCTypeOfMaterialsRows || payload, "", "MARCTypeOfMaterialsRow");
  var map = {};
  for (var i = 0; i < rows.length; i++) {
    var row = rows[i] || {};
...
var _materialTypesCacheMap = null;
var _materialTypesCacheLastCheck = 0;

function getMaterialTypesMap(app) {
  var now = new Date();
  if (_materialTypesCacheMap && (now.getTime() - _materialTypesCacheLastCheck < 60000)) {
    return _materialTypesCacheMap;
  }
  _materialTypesCacheLastCheck = now.getTime();
  ...
  if (cached) {
    if (typeof cached === "string") {
      try { map = JSON.parse(cached); } catch (e) { map = {}; }
    } else if (typeof cached === "object" && cached !== null) {
      // PocketBase JSON fields can come back as byte arrays (Uint8Array)
      if (cached.constructor && (cached.constructor.name === "Uint8Array" || cached.constructor.name === "Array")) {
        try {
          var str = "";
          for (var i = 0; i < cached.length; i++) {
            str += String.fromCharCode(cached[i]);
          }
          map = JSON.parse(str);
        } catch (e) { map = {}; }
      } else {
  ...
  if (map && typeof map === "object" && Object.keys(map).length > 0 && !isExpired) {
    _materialTypesCacheMap = map;
    return map;
  }

  // If no cache or expired, try to fetch it
  try {
    var auth = adminStaffAuth();
    var fetched = getMARCTypeOfMaterials(auth);
    if (fetched && Object.keys(fetched).length > 0) {
      settings.set("materialTypesCache", fetched);
      settings.set("materialTypesCacheUpdated", now.toISOString());
      app.save(settings);
      _materialTypesCacheMap = fetched;
      return fetched;
    }
  } catch (err) {
    if (app && app.logger) {
...
function normalizeBibSearchRow(app, row) {
  ...
  return {
    ...
    format: getBibFormatLabel(app, row),
...
function summarizeHoldability(holdings) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (barcode) {
      itemsTotal = 1;
      // For individual items, ItemsIn is usually 1 if available, 0 if not.
      // But we check the specific field if available.
      itemsIn = normalizeNumeric(row.ItemsIn || row.AvailableItems);
      if (itemsIn > 1) itemsIn = 1; // Sanity check for item-level row
    } else {
...
function getBib(app, staff, bibId) {
  ...
  return {
    ...
    format: getBibFormatLabel(app, payload, firstBibValue(rows, [17], ["Format"])),
...
module.exports = {
  ...
  getMARCTypeOfMaterials: getMARCTypeOfMaterials,
  placeHold: placeHold,
