...
function normalizeBibSearchRow(row) {
  ...
  return {
    ...
    title: firstRowValue(row, ["Title", "DisplayTitle", "FullTitle", "SortTitle"]),
...
function getBib(staff, bibId) {
  ...
  return {
    ...
    title: firstBibValue(rows, [35], ["Title"]),
    author: firstBibValue(rows, [18], ["Author"]),
    series: firstBibValue(rows, [19], ["Series"]),
...
function reconcileRecord(app, staff, record, bibId, selectedPolarisResult) {
  ...
  try {
    ...
    if (bibInfo) {
      ...
      if (selectedBibId && selectedBibId === String(bibId || "").trim() && selectedTitle && pTitle && selectedTitle !== pTitle) {
        if (app && app.logger) {
