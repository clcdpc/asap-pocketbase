...
function applyCatalogFoundWorkflow(app, record, data, staff) {
  ...
  records.appendSystemNote(
    record,
    "Staff selected Polaris BIB " + bibId + "; request marked ready for hold placement."
  );
...
function staffBibLookup(e) {
  ...
  try {
    ...
    info.holdingsSummary = polaris.summarizeHoldingsByLibrary(holdings, myLibraryOrgId);
