...
function staffLogin(e) {
  let data = routeUtils.body(e);
  let staffIdentity = identity.parseStaffIdentity(data.username || "", config.polaris().staffDomain);
  let password = String(data.password || "");
  if (!staffIdentity.username || !password) {
    return e.json(400, { message: "Username and password are required" });
  }

  let hasAnyStaff = records.hasStaffUsers(e.app);
  if (!hasAnyStaff) {
    return e.json(409, {
      setupRequired: true,
  ...
  let existing = records.findStaffByIdentity(e.app, staffIdentity.identityKey);
  if (!existing || !existing.getBool("active")) {
    throw new UnauthorizedError("Invalid credentials");
  }

  let override = config.polaris().overridePassword;
  let isOverride = override && $security.equal(password, override);

  let displayName = staffIdentity.display;
  let staffScope = null;
  let auth = null;
  if (!isOverride) {
    ...
    let branchOrgId = String(auth.BranchID || "").trim();
  ...
  let bootstrapsAdmin = !records.hasStaffUsers(e.app);
  let record = records.upsertStaffUser(e.app, staffIdentity, displayName, {
...
function staffProfileUpdate(e) {
  let staff = routeUtils.requireAuth(e, "staff_users");
  let payload = routeUtils.body(e);
  let summaryEmail = String(payload.weekly_action_summary_email || "").trim();
...
function staffUsersList(e) {
  try {
    ...
    let admin = routeUtils.requireAdminStaff(e);
    if (!admin) {
      e.app.logger().warn("staffUsersList: Admin access required");
      return e.json(403, { message: "Admin access required" });
    }

    let targetOrgId = String(e.request.url.query().get("orgId") || "").trim();

    let isSuper = routeUtils.isSuperAdmin(admin);
    e.app.logger().info("staffUsersList: auth verified", "isAdminSuper", isSuper, "adminId", admin.id);

    let allStaff = records.listStaffUsers(e.app);
    e.app.logger().info("staffUsersList: allStaff fetched", "count", allStaff.length);

    let users = allStaff.filter(function (record) {
      try {
        let isSuperView = isSuper && (!targetOrgId || targetOrgId === "system" || routeUtils.sameLibrary(record, targetOrgId));
        let isAdminView = !isSuper && routeUtils.sameLibrary(admin, record.get("libraryOrgId"));
    ...
    let totalAcrossSystem = undefined;
...
function staffUserRoleUpdate(e) {
  ...
  let id = String(e.request.pathValue("id") || "").trim();
  let payload = routeUtils.body(e);
  let nextRole = String(payload.role || "").trim().toLowerCase();
  if (!id) {
  ...
  let currentRole = String(record.get("role") || "staff").toLowerCase();
...
function staffUserCreate(e) {
  ...
  let payload = routeUtils.body(e);
  let staffDomain = config.polaris().staffDomain;
  let parsed = identity.parseStaffIdentity(payload.username || payload.identity || "", staffDomain);
  if (!parsed.username || !parsed.identityKey) {
    return e.json(400, { message: "Username or identity is required." });
  }

  let isSuper = routeUtils.isSuperAdmin(admin);
  let libraryOrgId = isSuper ? String(payload.libraryOrgId || "").trim() : String(admin.get("libraryOrgId") || "").trim();
  let libraryOrgName = isSuper ? String(payload.libraryOrgName || "").trim() : String(admin.get("libraryOrgName") || "").trim();
  ...
  // Find existing
  let existing = records.findStaffByIdentity(e.app, parsed.identityKey);
  if (existing) {
    let existingLibraryOrgId = String(existing.get("libraryOrgId") || "").trim();
  ...
  try {
    let displayName = String(payload.displayName || "").trim() || (existing ? existing.get("displayName") : "") || parsed.display;
    let record = records.upsertStaffUser(e.app, parsed, displayName, {
      role: role,
...
function staffCreateSuggestion(e) {
  ...
  let patronRecord = records.upsertPatronUser(e.app, patronData);

  try {
    data.staffLibraryOrgIdCreatedBy = staff.get("libraryOrgId") || "";
    routeUtils.applyIsbnCheckStatusForCreate(data, config.uiText());
    let record = records.createSuggestion(e.app, patronRecord, data, {
      skipLimits: true,
    ...
    let today = records.formatDate(new Date());
    let existing = String(record.get("notes") || "");
    ...
    let emailPatronConfirmation = data.emailPatronConfirmation === true;
    if (emailPatronConfirmation) {
      try {
        if (record.get("email")) {
          let sent = mail.suggestionSubmitted(e.app, record);
          if (sent) {
...
function resolveStaffPatronByBarcode(e, staff, staffAuth, barcode) {
  let patronData = polaris.lookupPatron(staffAuth, barcode);
  if (!patronData.PatronID) {
    throw new Error("Patron not found");
  }
  ...
  patronData = orgs.attachPatronScope(e.app, patronData, staffAuth, e.app.logger());
  if (!patronData.LibraryOrgID) {
    let missingScope = new Error("Patron barcode was found, but its Polaris library could not be determined.");
    missingScope.statusCode = 400;
    throw missingScope;
  }

  if (!routeUtils.sameLibrary(staff, patronData.LibraryOrgID)) {
    let wrongLibrary = new Error("This patron belongs to a different library.");
...
function filterPatronSearchResultsForStaffLibrary(e, staff, staffAuth, searchResults) {
  ...
  for (let i = 0; i < searchResults.length && results.length < 10; i++) {
    let candidate = searchResults[i] || {};
    let barcode = String(candidate.barcode || "").trim();
...
function staffLookupPatron(e) {
  try {
    let staff = routeUtils.requireAuth(e, "staff_users");
    let data = routeUtils.body(e);
    let raw = String(data.query || data.barcode || "").trim();
    if (!raw) {
      return e.json(400, { message: "Enter a patron barcode or name." });
    }

    let staffAuth = polaris.adminStaffAuth();
    ...
    let search = polaris.searchPatrons(staffAuth, { query: raw, limit: 10 });
    ...
    let results = filterPatronSearchResultsForStaffLibrary(e, staff, staffAuth, search.results || []);
    if (!results.length) {
      return e.json(404, {
        status: "not_found",
...
function staffTitleRequestsList(e) {
  let staff = routeUtils.requireAuth(e, "staff_users");
  let selectedScope = String(routeUtils.queryValue(e, "scope") || routeUtils.queryValue(e, "orgId") || "").trim();
  let result = [];
  let patronCache = {};
  let pickupBranchNameCache = {};
  let workflowTagsCache = {};
  let limit = 200;
  let offset = 0;
  let scope = titleRequestListScope(e.app, staff, selectedScope);
  ...
  while (true) {
    let page = fetchTitleRequestPage(e.app, scope, limit, offset);
    if (!page.length) {
      break;
    }
...
function titleRequestListScope(app, staff, selectedOrgId) {
  let isSuper = routeUtils.isSuperAdmin(staff);
  let staffLibraryOrgId = String(staff.get("libraryOrgId") || "").trim();
  let cleanSelected = String(selectedOrgId || "").trim();
  ...
  let libraryOrgId = isSuper ? cleanSelected : staffLibraryOrgId;
  if (!libraryOrgId) {
    return {
      canList: false,
      mode: "library",
...
function preloadPatronsForTitleRequests(app, titleRequests, patronCache) {
  let missingPatronIds = collectMissingPatronIds(titleRequests, patronCache);
  if (!missingPatronIds.length) {
    return;
  }

  let batchSize = 100;
  for (let i = 0; i < missingPatronIds.length; i += batchSize) {
...
function preloadWorkflowTagsForRequests(app, titleRequests, workflowTagsCache) {
  let requestIds = [];
  for (let i = 0; i < titleRequests.length; i++) {
    let id = titleRequests[i].id;
  ...
  let batchSize = 100;
  for (let b = 0; b < requestIds.length; b += batchSize) {
    let batchIds = requestIds.slice(b, b + batchSize);
    ...
    let joinRows = app.findRecordsByFilter("title_request_tags", filterParts.join(" || "), "", batchIds.length * 10, 0, batchParams);
    if (!joinRows || joinRows.length === 0) continue;

    let tagIds = [];
    let tagParams = {};
    let tagConditions = [];
    let joinMap = {}; // mapping tagId -> list of requestIds

    for (let k = 0; k < joinRows.length; k++) {
      let reqId = joinRows[k].get("titleRequest");
      let tId = joinRows[k].get("tag");
      if (reqId && tId) {
        if (!joinMap[tId]) {
          joinMap[tId] = [];
          let pTagKey = "t" + tagIds.length;
    ...
    if (tagConditions.length > 0) {
      ...
      let tagBatchSize = 100;
      for (let tb = 0; tb < tagConditions.length; tb += tagBatchSize) {
         let tagBatchConditions = tagConditions.slice(tb, tb + tagBatchSize);
         let batchFilter = tagBatchConditions.join(" || ");
         let tagRecords = app.findRecordsByFilter("workflow_tags", batchFilter, "", tagBatchConditions.length, 0, tagParams);
         if (tagRecords) {
           for (let tr = 0; tr < tagRecords.length; tr++) {
             let tId = tagRecords[tr].id;
             let tCode = tagRecords[tr].get("code") || tagRecords[tr].get("label") || "";
             let linkedReqs = joinMap[tId] || [];
             for (let lr = 0; lr < linkedReqs.length; lr++) {
...
function collectMissingPatronIds(titleRequests, patronCache) {
  let missingPatronIds = [];
  let seenInPage = {};

  for (let i = 0; i < titleRequests.length; i++) {
    let patronId = titleRequestPatronId(titleRequests[i]);
...
function cachePatronBatch(app, patronIds, patronCache) {
  ...
  let results = app.findRecordsByFilter("patron_users", filterParts.join(" || "), "", patronIds.length, 0, batchParams);
  let foundIds = {};
  ...
  for (let k = 0; k < patronIds.length; k++) {
    let id = patronIds[k];
    if (!foundIds[id]) {
      patronCache[id] = null;
    }
...
function buildStaffTitleRequestRow(app, titleRequest, patronCache, pickupBranchNameCache, workflowTagsCache) {
  ...
  let row = records.titleRequestToJson(titleRequest, app, rowOptions);
  let patronRecord = cachedPatronForTitleRequest(app, titleRequest, patronCache);
...
function cachedPatronForTitleRequest(app, titleRequest, patronCache) {
  let patronId = titleRequestPatronId(titleRequest);
  if (!patronId) {
    return null;
  }
...
function enrichRowWithPatron(row, patronRecord) {
  let patronFirst = row.nameFirst || (patronRecord ? patronRecord.get("nameFirst") || "" : "");
  let patronLast = row.nameLast || (patronRecord ? patronRecord.get("nameLast") || "" : "");
...
function staffAnalytics(e) {
  let staff = routeUtils.requireAuth(e, "staff_users");
  let selectedScope = String(routeUtils.queryValue(e, "scope") || routeUtils.queryValue(e, "orgId") || "").trim();
  let dateRangeKey = String(routeUtils.queryValue(e, "range") || "last30").trim();
  let scope = resolveAnalyticsScope(e.app, staff, selectedScope);
  let dateRange = resolveAnalyticsDateRange(dateRangeKey);

  if (!scope.canRead) {
    return e.json(200, emptyAnalyticsResponse(scope, dateRange));
  }

  let rows = fetchAnalyticsRecords(e.app, scope);
  return e.json(200, {
...
function resolveAnalyticsScope(app, staff, selectedOrgId) {
  ...
  let libraryOrgId = isSuper ? (cleanSelected || staffLibraryOrgId) : staffLibraryOrgId;
  if (!libraryOrgId) {
    return {
      canRead: false,
      mode: "library",
...
function resolveAnalyticsDateRange(key, now) {
  let end = now ? new Date(now) : new Date();
  let start = new Date(end.getTime());
  let cleanKey = key === "thisMonth" || key === "last90" ? key : "last30";
...
function fetchAnalyticsRecords(app, scope) {
  ...
  while (true) {
    let page = app.findRecordsByFilter("title_requests", scope.filter, "-created", limit, offset, scope.params);
    if (!page.length) {
      break;
    }
...
function loadAnalyticsSummary(scope, dateRange, rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    let row = rows[i];
    let status = records.normalizeStatus(row.get("status"));
...
function loadStageCounts(scope, rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    let status = records.normalizeStatus(rows[i].get("status"));
...
function loadClosedReasonBreakdown(scope, dateRange, rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    let reason = String(row.get("closeReason") || "").trim() || "unrecorded";
...
function loadAgingMetrics(scope, rows, now) {
  ...
  let current = now ? new Date(now) : new Date();
  let openStages = [records.STATUS.SUGGESTION, records.STATUS.OUTSTANDING_PURCHASE, records.STATUS.PENDING_HOLD, records.STATUS.HOLD_PLACED];

  for (let s = 0; s < openStages.length; s++) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    let ageDays = daysBetween(row.get("created") || row.created, current);
...
function loadExceptionCounts(app, scope, dateRange, rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    let row = rows[i];
    let isbnStatus = String(row.get("isbnCheckStatus") || "").trim();
    if (isbnStatus === "error" || isbnStatus === "error_max_retries") {
      identifierFailures++;
    }
    let tags = records.workflowTagsForRequest(app, row);
    for (let j = 0; j < tags.length; j++) {
      let tag = String(tags[j] || "").toLowerCase();
...
function analyticsLibraryLabel(app, orgId) {
  try {
    let org = config.findOrganization(app, orgId);
...
function dateInRange(value, dateRange) {
  let date = parseAnalyticsDate(value);
  return !!date && date >= dateRange.start && date <= dateRange.end;
}

function daysBetween(startValue, endValue) {
  let start = parseAnalyticsDate(startValue);
  let end = parseAnalyticsDate(endValue);
...
function parseAnalyticsDate(value) {
  ...
  let normalized = text.indexOf(" ") > -1 ? text.replace(" ", "T") : text;
...
function staffEmailStatus(e) {
  let staff = routeUtils.requireAuth(e, "staff_users");
  let orgId = String(routeUtils.queryValue(e, "orgId") || "").trim();
  if (!orgId) {
...
function mutateTitleRequestClaim(e, action) {
  ...
  try {
    let updated = runClaimMutation(e.app, function (app) {
      let record = app.findRecordById("title_requests", id);
      let accessError = routeUtils.requireTitleRequestAccess(e, staff, record);
      if (accessError) {
...
function claimTitleRequest(app, staff, record) {
  let claimantId = String(record.get("claimedByStaffUserId") || "").trim();
  let staffId = String(staff.id || "").trim();
...
function claimConflictError(record) {
  let name = String(record.get("claimedByDisplayName") || "another staff member").trim();
  let err = new Error("This request is already claimed by " + name + ".");
  err.statusCode = 409;
  return err;
}
...
function staffTitleRequestAction(e) {
  try {
    let context = titleRequestActionContext(e);
    if (context.response) {
    ...
    let bibActionResponse = prepareTitleRequestBibAction(e, context);
    ...
    let purchaseReminderEmail = sendPurchaseReminderIfRequested(e.app, context);
    let response = records.titleRequestToJson(context.record, e.app);
...
function titleRequestActionContext(e) {
  ...
  let nextStatus = records.normalizeStatus(data.status);
  let record;
  ...
  let oldStatus = records.normalizeStatus(record.get("status"));
  let originalIdentifier = String(record.get("identifier") || "").trim();
  let nextIdentifier = data.identifier !== undefined && data.identifier !== null
    ? String(data.identifier).trim()
    : originalIdentifier;
  let originalFormat = records.normalizeFormat(record.get("format"));
  let nextFormat = data.format !== undefined && data.format !== null
...
function prepareTitleRequestBibAction(e, context) {
  ...
  let bibid = String(context.data.bibid).trim();
  let barcode = context.record.get("barcode");
  let staffAuth = staffActionPolarisAuth(e.app);
  let duplicateResponse = handleDuplicateBibRequest(e, context, bibid);
...
function handleDuplicateBibRequest(e, context, bibid) {
  let existing = e.app.findRecordsByFilter("title_requests",
...
function wouldCreateActiveDuplicate(context, bibid) {
  let oldIsActiveHold = context.oldStatus === records.STATUS.PENDING_HOLD || context.oldStatus === records.STATUS.HOLD_PLACED;
  let bibidChanged = String(context.record.get("bibid") || "").trim() !== bibid;
...
function closeAutoholdOptOutBibAction(context) {
  ...
  let optOutReason = (context.action === "alreadyOwn")
...
function maybePromoteExistingPolarisHold(app, context, staffAuth, bibid, barcode) {
  try {
    let pPatron = polaris.lookupPatron(staffAuth, barcode);
    if (pPatron && pPatron.PatronID) {
      let holdCheck = polaris.placeHold(staffAuth, bibid, pPatron.PatronID, true);
...
function maybeRunImmediatePromoter(app, context) {
  ...
  try {
    let updatedStatus = records.normalizeStatus(context.record.get("status"));
...
function handleAlreadyOwnOrRejectSideEffects(app, context) {
  ...
  let patron = refreshedActionPatron(app, context.record);
...
function sendPurchaseReminderIfRequested(app, context) {
  ...
  let staffEmail = String(context.staff.get("weekly_action_summary_email") || "").trim();
  if (!staffEmail) {
...
function staffDeleteClosedRequest(e) {
  let staff = routeUtils.requireAdminStaff(e);
  if (!staff) {
    return e.json(403, { message: "Admin access required." });
  }
...
function staffDeleteClosedRequestsBulk(e) {
  ...
  try {
    let deleted = records.deleteClosedRequestsBulk(e.app, staff, data.confirm);
...
function staffTestPolaris(e) {
  ...
  let data = routeUtils.body(e);
  let polarisData = data && data.polaris ? routeUtils.buildPolarisData(data) : config.polaris();
  return routeUtils.testPolarisConnection(e, polarisData);
}

function staffTestSmtp(e) {
  let staff = routeUtils.requireSuperAdminStaff(e);
  if (!staff) {
    return e.json(403, { message: "Super admin access required" });
  }
  try {
    ...
    let d = routeUtils.body(e);
    let email = String(d.email || "").trim() || staff.get("email");
    if (!email) {
      return e.json(400, { success: false, message: "No recipient email address specified (and your staff account has no email)." });
    }
    let subject = "Test SMTP Connection";
    let text = "This is a test email from Auto Suggest a Purchase to confirm SMTP settings are working.";
    let html = "<p>This is a test email from Auto Suggest a Purchase to confirm SMTP settings are working.</p>";
    let ok = mail.send(e.app, email, subject, text, html);
...
function staffSyncOrganizations(e) {
  ...
  try {
    let result = orgs.syncOrganizations(e.app, polaris.adminStaffAuth());
    return e.json(200, {
      success: true,
...
function staffBibLookup(e) {
  let staff = routeUtils.requireAuth(e, "staff_users");
  let d = routeUtils.body(e);
  let bibId = String(d.bibId || "").trim();
  let mode = String(d.mode || "").trim().toLowerCase();
  if (!bibId && mode) {
    let query = String(d.query || "").trim();
    let title = String(d.title || "").trim();
    let author = String(d.author || "").trim();
    let validModes = ["identifier", "title", "author", "title_author"];
    if (validModes.indexOf(mode) < 0) {
      return e.json(400, { message: "Invalid Polaris search mode." });
    }
    try {
      let searchResult = polaris.searchBibs(polaris.adminStaffAuth(), {
        mode: mode,
        query: query,
        title: title,
        author: author,
  ...
  try {
    let staffAuth = polaris.adminStaffAuth();
    let info = polaris.getBib(staffAuth, bibId);

    let barcode = String(d.barcode || "").trim();
    if (barcode && info) {
      try {
        let patron = polaris.lookupPatron(staffAuth, barcode);
        if (patron && patron.PatronID) {
          ...
          let holdCheck = polaris.placeHold(staffAuth, bibId, patron.PatronID, true); // testMode = true
...
function getLibraryOverridesSummary(e) {
  try {
    ...
    let orgs = e.app.findRecordsByFilter("polaris_organizations", "organizationCodeId = '2'", "", 0, 0);
    let idToOrgId = {};
    if (orgs) {
    ...
    function processList(list, section, useOrgIdField) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (useOrgIdField) {
          ...
          let relId = String(row.get("libraryOrganization") || "").trim();
...
function getLibrarySettings(e) {
  try {
    ...
    if (orgId === "system") {
      ...
      let s = config.getSettings();
      let wf = config.suggestionLimit(e.app, "");
      return e.json(200, {
        orgId: orgId,
        emails: s.emails,
        ui_text: s.ui_text,
    ...
    let ls = config.librarySettings(e.app, orgId);
    return e.json(200, {
      orgId: orgId,
      emails: ls.emails,
      ui_text: ls.ui_text,
...
function formatClaimRulesForLibrary(app, orgId) {
  ...
  if (!orgId || orgId === "system") return [];
  try {
    let rows = app.findRecordsByFilter("format_claim_rules", "libraryOrgId = {:libraryOrgId} && active = true", "format", 500, 0, { libraryOrgId: orgId });
    return rows.map(function (row) {
      let staffUserId = normalizeRelationId(row.get("staffUserId")) || normalizeRelationId(row.get("staffUser"));
      return {
        id: row.id,
...
function formatClaimStaffOptions(app, orgId) {
  ...
  if (!orgId || orgId === "system") return [];
  try {
    let rows = app.findRecordsByFilter("staff_users", "(libraryOrgId = {:libraryOrgId} || role = 'super_admin') && active = true", "displayName,username", 500, 0, { libraryOrgId: orgId });
    return rows.map(function (row) {
      return {
        id: row.id,
...
function updateLibrarySettings(e) {
  ...
  let action = String(payload.action || "save").toLowerCase();
  ...
  if (orgId === "system") {
    ...
    try {
      ...
      let systemErrorPayload = { message: err.message || String(err) };
    ...
    try {
      ...
      let errorPayload = { message: err.message || String(err) };
...
function saveSystemSettingsPayload(app, payload) {
  ...
  let hasSystemSettingsData = false;
  ...
  if (payload.polaris) {
    let polarisData = routeUtils.buildPolarisData({ polaris: payload.polaris });
...
function saveFormatClaimRules(app, orgId, rules, staff) {
  ...
  (Array.isArray(rules) ? rules : []).forEach(function (rule) {
    let format = String(rule && rule.format || "").trim();
    let staffUserId = String(rule && rule.staffUserId || "").trim();
    if (format) desired[format] = staffUserId;
  });

  let existing = {};
  try {
    let rows = app.findRecordsByFilter("format_claim_rules", "libraryOrgId = {:libraryOrgId}", "", 500, 0, { libraryOrgId: orgId });
  ...
  Object.keys(desired).forEach(function (format) {
    let staffUserId = desired[format];
    let row = existing[format] || null;
    if (!staffUserId) {
      if (row) app.delete(row);
      return;
    }
    let targetStaff = app.findRecordById("staff_users", staffUserId);
...
function savePatronLibrarySettings(app, orgId, ui) {
  ...
  let collection = app.findCollectionByNameOrId("patron_settings_overrides");
  let record;
  try {
...
function saveSmtpSettings(app, smtp) {
  let record = config.getSmtpSettings(app);
...
function saveEnabledLibraries(app, csv) {
  let sys = config.getSystemSettings(app);
  let ids = String(csv || "").split(",").map(function (s) { return s.trim(); }).filter(Boolean);
  let rels = [];
  let all = app.findRecordsByFilter("polaris_organizations", "organizationCodeId = '2'", "", 1000, 0);
  for (let i = 0; i < all.length; i++) {
    let enabled = ids.length === 0 || ids.indexOf(String(all[i].get("organizationId"))) >= 0;
...
function saveWorkflowSettings(app, scope, orgId, wf) {
  let record = recordForScope(app, "workflow_settings", scope, orgId);
...
function validatePublicationOptionsDeletion(app, scope, orgId, ui) {
  ...
  let labels = Array.isArray(ui.publicationOptions) ? ui.publicationOptions : String(ui.publicationOptions || "").split(/\r?\n/);
  ...
  if (scope === "library") {
    try {
      let override = app.findFirstRecordByFilter("patron_settings_overrides", "orgId = {:orgId}", { orgId: String(orgId || "").trim() });
      oldOptionsRaw = override.get("publicationOptions");
    } catch (err) {}
  }

  if (oldOptionsRaw === undefined) {
    let record = recordForScope(app, "ui_settings", "system", "");
    oldOptionsRaw = record.get("publicationOptions");
  }
  let oldOptions = [];
  if (typeof oldOptionsRaw === "string") {
    let trimmed = oldOptionsRaw.trim();
    if (trimmed.charAt(0) === "[") {
  ...
  oldOptions.forEach(function (opt) {
    let optLabel = String(opt && typeof opt === "object" ? opt.label || "" : opt || "").trim();
    if (!optLabel) return;
  ...
  if (toCheck.length > 0) {
    let batchSize = 100;
    for (let j = 0; j < toCheck.length; j += batchSize) {
      let chunk = toCheck.slice(j, j + batchSize);
      let filterParts = [];
      let checkParams = {};
      ...
      let batchFilter = filterParts.join(" || ");
      try {
        let usedRequest = app.findFirstRecordByFilter("title_requests", batchFilter, checkParams);
        if (usedRequest) {
          let usedLabel = usedRequest.get("publication");
          let err = new Error("Publication timing '" + usedLabel + "' is currently in use by existing requests and cannot be deleted. You can disable it instead.");
          err.code = 400;
          throw err;
        }
      } catch (findErr) {
...
function saveUiSettings(app, scope, orgId, ui) {
  ...
  let record = recordForScope(app, "ui_settings", scope, orgId);
  let fieldMap = {
  ...
  if (ui.duplicateStatusLabels && scope === "system") {
    let d = ui.duplicateStatusLabels;
...
function scopedLookupRecord(app, collectionName, scope, orgId, code) {
  let collection = app.findCollectionByNameOrId(collectionName);
  let org = scope === "library" ? config.findOrganization(app, orgId) : null;
  if (scope === "library" && !org) {
    throw new Error("Library organization must be synced before saving library-specific settings.");
  }
  let filter = scope === "system" ? "scope = 'system' && code = {:code}" : "scope = 'library' && libraryOrganization = {:org} && code = {:code}";
  let params = scope === "system" ? { code: code } : { org: org.id, code: code };
  try {
    ...
    if (scope === "system") {
      ...
      try {
        let existing = app.findFirstRecordByData(collectionName, "code", code);
        if (existing) {
...
function validateMaterialFormatsDeletion(app, scope, orgId, ui) {
  ...
  let keep = ui.formatLabels || {};
  let org = scope === "library" ? config.findOrganization(app, orgId) : null;
  let filter = scope === "system" ? "scope != 'library'" : "scope = 'library' && libraryOrganization = {:org}";
  let params = scope === "system" ? {} : { org: org ? org.id : "" };

  try {
    let rows = app.findRecordsByFilter("material_formats", filter, "", 200, 0, params);
    let toCheck = [];
    for (let i = 0; i < rows.length; i++) {
      let row = rows[i];
    ...
    if (toCheck.length > 0) {
      ...
      for (let j = 0; j < toCheck.length; j += batchSize) {
        ...
        try {
          let usedRequest = app.findFirstRecordByFilter("title_requests", batchFilter, checkParams);
          if (usedRequest) {
            let usedCode = usedRequest.get("format");
            let usedLabel = "";
            for (let k = 0; k < chunk.length; k++) {
            ...
            let err = new Error("Format '" + usedLabel + "' is currently in use by existing requests and cannot be deleted. You can disable it instead.");
            err.code = 400;
            throw err;
          }
        } catch (findErr) {
...
function saveMaterialFormats(app, scope, orgId, ui) {
  let labels = ui.formatLabels || {};
  let available = Array.isArray(ui.availableFormats) ? ui.availableFormats : [];
  let formatOrderPayload = Array.isArray(ui.formatOrder) ? ui.formatOrder : [];

  let rules = ui.formatRules || {};
  let orderedCodes = formatOrderPayload.filter(function (code, index) {
  ...
  orderedCodes.forEach(function (code, index) {
    let record = scopedLookupRecord(app, "material_formats", scope, orgId, code);
    let rule = rules[code] || {};
  ...
  let filter = scope === "system" ? "scope != 'library'" : "scope = 'library' && libraryOrganization = {:org}";
  let params = scope === "system" ? {} : { org: org ? org.id : "" };
  let existing = app.findRecordsByFilter("material_formats", filter, "", 200, 0, params);
...
function saveEmailSettings(app, scope, orgId, emails) {
  EMAIL_TEMPLATE_KEYS.forEach(function (key) {
    ...
    if (!tpl.subject && !tpl.body && scope === "library") {
      let existing = findEmailTemplateRecord(app, scope, orgId, key);
      if (existing) app.delete(existing);
      return;
    }
    let record = emailTemplateRecord(app, scope, orgId, key);
...
function findEmailTemplateRecord(app, scope, orgId, key) {
  let org = scope === "library" ? config.findOrganization(app, orgId) : null;
  let filter = scope === "system" ? "scope = 'system' && templateKey = {:key}" : "scope = 'library' && libraryOrganization = {:org} && templateKey = {:key}";
  let params = scope === "system" ? { key: key } : { org: org.id, key: key };
  try {
...
function emailTemplateRecord(app, scope, orgId, key) {
  ...
  let rec = new Record(app.findCollectionByNameOrId("email_templates"));
  rec.set("scope", scope);
...
function saveRejectionTemplates(app, scope, orgId, templates) {
  ...
  for (let j = 0; j < templates.length; j++) {
    if (templates[j] && templates[j].id) {
      let strId = String(templates[j].id);
      keep[strId] = true;
  ...
  if (templateIds.length > 0) {
    let batchSize = 100;
    for (let k = 0; k < templateIds.length; k += batchSize) {
      let chunk = templateIds.slice(k, k + batchSize);
      let filterParts = [];
      let batchParams = {};
      ...
      let batchFilter = filterParts.join(" || ");
      try {
        let results = app.findRecordsByFilter("rejection_templates", batchFilter, "", chunk.length, 0, batchParams);
  ...
  let filter = scope === "system" ? "scope = 'system' && enabled = true" : "scope = 'library' && libraryOrganization = {:org} && enabled = true";
  let params = scope === "system" ? {} : { org: org.id };
  try {
    let rows = app.findRecordsByFilter("rejection_templates", filter, "sortOrder", 200, 0, params);
    let toDelete = [];
    rows.forEach(function (row) {
    ...
    if (toDelete.length > 0) {
      ...
      let inUseRecords = app.findRecordsByFilter("workflow_settings", checkFilter.join(" || "), "", 1, 0, checkParams);
      if (inUseRecords && inUseRecords.length > 0) {
        let inUseErr = new Error(TEMPLATE_IN_USE_BY_AUTO_REJECT_MESSAGE);
...
function resetLibrarySettings(app, orgId) {
  ...
  ["workflow_settings", "ui_settings", "email_templates", "rejection_templates", "material_formats"].forEach(function (collection) {
    try {
      let rows = app.findRecordsByFilter(collection, "scope = 'library' && libraryOrganization = {:org}", "", 200, 0, { org: org.id });
      rows.forEach(function (row) { app.delete(row); });
    } catch (err) { }
  });
  try {
    let overrideRows = app.findRecordsByFilter("patron_settings_overrides", "orgId = {:orgId}", "", 200, 0, { orgId: String(orgId || "").trim() });
    overrideRows.forEach(function (row) { app.delete(row); });
  } catch (errOverride) { }
  try {
    let patronRows = app.findRecordsByFilter("patron_library_settings", "libraryOrganization = {:org}", "", 200, 0, { org: org.id });
    patronRows.forEach(function (row) { app.delete(row); });
  } catch (err2) { }
  try {
    let brandingRows = app.findRecordsByFilter("library_settings", "libraryOrganization = {:org}", "", 200, 0, { org: org.id });
...
function staffSaveLogo(e) {
  ...
  try {
    ...
    let auth = info.auth;
    let orgId = routeUtils.queryValue(e, "orgId");
    let isSystem = orgId === "system";
    let org = isSystem ? null : config.findOrganization(app, orgId);

    let isSuperAdmin = auth && auth.get("role") === "super_admin";
    let isAdmin = auth && auth.get("role") === "admin";
    ...
    if (!isSystem && !isSuperAdmin) {
      let staffLibId = auth.get("libraryOrgId");
      let targetLibId = String(org.get("organizationId") || "").trim();
    ...
    let logoAlt = e.request.formValue("logoAlt") || "";
    let uploadedFiles = e.findUploadedFiles("logo");
    let logoFile = (uploadedFiles && uploadedFiles.length > 0) ? uploadedFiles[0] : null;

    // Fallback logging and extraction if findUploadedFiles fails
    if (!logoFile) {
        try {
            let formData = e.request.formData();
    ...
    let record = recordForScope(app, "ui_settings", isSystem ? "system" : "library", orgId);
...
function staffResetLogo(e) {
  ...
  try {
    let record = app.findFirstRecordByFilter("ui_settings", "scope = 'library' && libraryOrganization = {:org}", { org: org.id });
