...
function staffSaveLogo(e) {
  ...
  try {
    ...
    var logoAlt = (info.body && info.body.logoAlt) || "";
    var logoFile = null;
    if (info.files && info.files.logo && info.files.logo.length > 0) {
      logoFile = info.files.logo[0];
    }

    app.logger().debug("Logo Upload: Processing", "orgId", orgId, "isSystem", isSystem, "hasFile", !!logoFile);

    if (isSystem) {
      var record = app.findRecordById("ui_settings", "uisettings00010");
      record.set("logoAlt", logoAlt);
      if (logoFile) record.set("logo", logoFile);
      app.save(record);
    } else {
      var brandingRecord = null;
      try {
        brandingRecord = app.findFirstRecordByFilter("library_settings", "libraryOrganization = {:org}", { org: org.id });
        app.logger().debug("Logo Upload: Found existing library_settings record", "id", brandingRecord.id);
      } catch (e) {
        app.logger().debug("Logo Upload: Creating new library_settings record for library", "orgId", orgId);
        var collection = app.findCollectionByNameOrId("library_settings");
        brandingRecord = new Record(collection);
        brandingRecord.set("libraryOrganization", org.id);
        brandingRecord.set("libraryOrgId", String(orgId || "").trim());
        brandingRecord.set("libraryOrgName", String(org.get("name") || "").trim());
      }

      brandingRecord.set("logoAlt", logoAlt);
      
      if (logoFile) {
        app.logger().debug("Logo Upload: Attaching file to library_settings", "fileName", logoFile.name);
        brandingRecord.set("logo", logoFile);
      }
      
      app.save(brandingRecord);
    }
...
function staffResetLogo(e) {
  ...
  try {
    var record = app.findFirstRecordByFilter("library_settings", "libraryOrganization = {:org}", { org: org.id });
    app.delete(record);
  } catch (err) { }
