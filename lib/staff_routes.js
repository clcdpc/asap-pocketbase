...
function staffLogin(e) {
  ...
  if (!isOverride) {
    try {
      ...
      e.app.logger().error("Staff login polaris authentication failed", "error", String(err));
      throw new UnauthorizedError("Invalid credentials");
    }
...
function saveMaterialFormats(app, scope, orgId, ui) {
  ...
  var order = Array.isArray(ui.formatOrder) ? ui.formatOrder : [];
  var rules = ui.formatRules || {};
  var orderedCodes = order.filter(function (code, index) {
    return Object.prototype.hasOwnProperty.call(labels, code) && order.indexOf(code) === index;
  });
