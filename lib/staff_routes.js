    ...
    if (data.bibid) {
      ...
      if (nextStatus === records.STATUS.PENDING_HOLD || (oldStatus === records.STATUS.OUTSTANDING_PURCHASE && data.bibid)) {
        if (record.getBool("autohold") === false) {
          ...
          records.appendSystemNote(record, "Closed without hold because BIB ID was entered and patron opted out of automatic hold placement.");
