...
function staffUsersList(e) {
  ...
  var targetOrgId = String(e.request.queryParam("orgId") || "").trim();
  var isSuper = routeUtils.isSuperAdmin(admin);

  var users = records.listStaffUsers(e.app).filter(function (record) {
    var isSuperView = isSuper && (!targetOrgId || targetOrgId === "system" || routeUtils.sameLibrary(record.get("libraryOrgId"), targetOrgId));
  ...
  if (isSuper && targetOrgId && targetOrgId !== "system") {
    totalAcrossSystem = records.listStaffUsers(e.app).length;
  }
