...
function staffTitleRequestsList(e) {
  ...
  var limit = 200;
  var offset = 0;
  var scope = titleRequestListScope(staff);
  ...
  while (true) {
    ...
    for (var i = 0; i < page.length; i++) {
      result.push(buildStaffTitleRequestRow(e.app, page[i], patronCache, pickupBranchNameCache));
    }
  ...
  return e.json(200, {
    items: result,
    scope: titleRequestListResponseScope(staff)
  });
}

function titleRequestListScope(staff) {
...
function buildStaffTitleRequestRow(app, titleRequest, patronCache, pickupBranchNameCache) {
  var row = records.titleRequestToJson(titleRequest, app);
...
function titleRequestListResponseScope(staff) {
  return {
...
function staffTitleRequestAction(e) {
  try {
    ...
    if (data.bibid) {
      ...
      // If moving to Pending hold, OR if this is a manual BIB entry for a "Do Not Auto Hold" request, handle immediate closure.
      if (nextStatus === records.STATUS.PENDING_HOLD || (oldStatus === records.STATUS.OUTSTANDING_PURCHASE && data.bibid)) {
        if (record.getBool("autohold") === false) {
          nextStatus = records.STATUS.CLOSED;
          data.status = nextStatus;
          data.closeReason = records.CLOSE_REASON.PURCHASED_NO_HOLD;
          var optOutReason = (action === "alreadyOwn")
          ...
          records.appendSystemNote(record, optOutReason);
        } else if (nextStatus === records.STATUS.PENDING_HOLD) {
          try {
    ...
    // Explicitly check if we just moved to CLOSED due to autohold=false
    if (nextStatus === records.STATUS.CLOSED && !data.closeReason && record.getBool("autohold") === false && data.bibid) {
       data.closeReason = records.CLOSE_REASON.PURCHASED_NO_HOLD;
    }
    ...
    if (shouldRunImmediatePromoter) {
      try {
        ...
        if (updatedStatus === records.STATUS.SUGGESTION || config.suggestionLimit(e.app, "").autoPromote !== false) {
...
function saveWorkflowSettings(app, scope, orgId, wf) {
  ...
  ["suggestionLimit", "suggestionLimitMessage", "outstandingTimeoutEnabled", "outstandingTimeoutDays", "outstandingTimeoutSendEmail", "holdPickupTimeoutEnabled", "holdPickupTimeoutDays", "pendingHoldTimeoutEnabled", "pendingHoldTimeoutDays", "autoPromote", "commonAuthorsEnabled", "commonAuthorsList", "commonAuthorsMessage", "commonAuthorsLabel", "commonAuthorsHelp", "allowPatronAutoholdOptOut", "externalSearch1Enabled", "externalSearch1Label", "externalSearch1UrlTemplate", "externalSearch2Enabled", "externalSearch2Label", "externalSearch2UrlTemplate", "externalSearch3Enabled", "externalSearch3Label", "externalSearch3UrlTemplate"].forEach(function (key) {
  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (code && !Object.prototype.hasOwnProperty.call(keep, code)) {
        var label = row.get("label") || code;
        try {
          var usedRequest = app.findFirstRecordByFilter("title_requests", "format = {:code}", { code: code });
          if (usedRequest) {
            var err = new Error("Format '" + label + "' is currently in use by existing requests and cannot be deleted. You can disable it instead.");
            err.code = 400;
            throw err;
          }
        } catch (findErr) {
