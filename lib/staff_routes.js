...
function getLibrarySettings(e) {
  try {
    ...
    // formatClaimRules is intentionally library-only. Enforce a concrete
    // library org scope on all load paths to avoid accidental system usage.
    var libraryOrgId = assertLibraryScopedOrgId(orgId, "load format auto-claim settings");
    var ls = config.librarySettings(e.app, libraryOrgId);
    return e.json(200, {
      orgId: libraryOrgId,
      emails: ls.emails,
      ui_text: ls.ui_text,
      workflow: workflowWithEnabled(e.app, ls.workflow),
      formatClaimRules: formatClaimRulesForLibrary(e.app, libraryOrgId),
      formatClaimStaffOptions: formatClaimStaffOptions(e.app, libraryOrgId),
      leapBibUrlPattern: ls.leapBibUrlPattern || "",
      emailStatus: config.emailStatus(e.app, libraryOrgId),
      organizationSync: organizationSyncStatus(e.app),
      isOverride: hasLibraryOverride(e.app, libraryOrgId)
    });
  } catch (err) {
  ...
  try {
    ...
    return rows.map(function (row) {
      return {
        ...
        staffUserId: row.get("staffUserId") || row.get("staffUser") || "",
...
function formatClaimStaffOptions(app, orgId) {
  ...
  if (!orgId || orgId === "system") return [];
  try {
    var rows = app.findRecordsByFilter("staff_users", "libraryOrgId = {:libraryOrgId} && active = true", "displayName,username", 500, 0, { libraryOrgId: orgId });
    return rows.map(function (row) {
      return {
        id: row.id,
...
function saveLibraryScopedSettings(app, orgId, payload) {
  // Settings handled here are persisted as library-scoped overrides.
  var libraryOrgId = assertLibraryScopedOrgId(orgId, "save library-scoped settings");
  saveWorkflowSettings(app, "library", libraryOrgId, payload.workflow || {});
  saveUiSettings(app, "library", libraryOrgId, payload.ui_text || {});
  saveEmailSettings(app, "library", libraryOrgId, payload.emails || {});
  if (Object.prototype.hasOwnProperty.call(payload, "formatClaimRules")) {
    saveFormatClaimRules(app, libraryOrgId, payload.formatClaimRules || [], payload._staffUser || null);
  }
}

function saveFormatClaimRules(app, orgId, rules, staff) {
  // Format auto-claim is library-only and must never use system scope.
  var libraryOrgId = assertLibraryScopedOrgId(orgId, "save format auto-claim settings");
  var org = config.findOrganization(app, libraryOrgId);
  ...
  (Array.isArray(rules) ? rules : []).forEach(function (rule) {
    var format = records.normalizeFormat(rule && rule.format);
  ...
  var existing = {};
  try {
    var rows = app.findRecordsByFilter("format_claim_rules", "libraryOrgId = {:libraryOrgId}", "", 500, 0, { libraryOrgId: libraryOrgId });
  ...
  Object.keys(desired).forEach(function (format) {
    ...
    if (String(targetStaff.get("libraryOrgId") || "").trim() !== libraryOrgId && String(targetStaff.get("role") || "") !== "super_admin") {
    ...
    row.set("libraryOrgId", libraryOrgId);
...
function assertLibraryScopedOrgId(orgId, actionLabel) {
  var libraryOrgId = String(orgId || "").trim();
  if (!libraryOrgId || libraryOrgId === "system") {
    throw new Error("Format auto-claim is library-only. A non-system library orgId is required to " + (actionLabel || "continue") + ".");
  }
  return libraryOrgId;
}
