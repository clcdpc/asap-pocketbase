...
function staffSaveLogo(e) {
  ...
  try {
    ...
    if (isSystem) {
      ...
      try {
        brandingRecord = app.findFirstRecordByFilter("ui_settings", "scope = 'library' && libraryOrganization = {:org}", { org: org.id });
        app.logger().debug("Logo Upload: Found existing ui_settings record", "id", brandingRecord.id);
      } catch (e) {
        app.logger().debug("Logo Upload: Creating new ui_settings record for library", "orgId", orgId);
        var collection = app.findCollectionByNameOrId("ui_settings");
        brandingRecord = new Record(collection);
        brandingRecord.set("scope", "library");
