...
function Test-RecipientDomain([object] $Value) {
    ...
    try { $ascii = [Globalization.IdnMapping]::new().GetAscii($domain) }
    catch { return $false }
...
function Test-ExactKeys([object] $Object, [string[]] $ExpectedKeys) {
    if ($null -eq $Object) { return $false }
    $actual = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($name in Get-ObjectPropertyNames $Object) { [void]$actual.Add($name) }
    $expected = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($name in $ExpectedKeys) { [void]$expected.Add($name) }
    return $actual.SetEquals($expected)
}

function Test-CanonicalConnectionString([object] $Actual, [string] $Expected) {
    if ($null -eq $Actual) { return $false }
    return ([string]$Actual).Trim().Equals($Expected, [StringComparison]::OrdinalIgnoreCase)
}
...
function Test-AppConfiguration([object] $Configuration, [string] $ExpectedConnectionString, [string] $ExpectedKeysPath, [string] $ExpectedLogsPath, [Security.Cryptography.X509Certificates.X509Certificate2] $Certificate) {
    ...
    foreach ($name in @('AsapDatabase', 'HangfireDatabase')) {
        if (-not (Test-CanonicalConnectionString (Get-JsonValue $connections $name) $ExpectedConnectionString)) { $errors.Add("ConnectionStrings.$name is not the canonical bootstrap-owned SQL connection string.") }
    }
    ...
    $tenantIdsValue = Get-JsonValue $entra 'AllowedTenantIds'
    $tenantIds = if ($null -eq $tenantIdsValue) { @() } else { @($tenantIdsValue) }
    if ($tenantIds.Count -eq 0) { $errors.Add('Authentication.Entra.AllowedTenantIds is empty.') }
    else {
    ...
    if ($null -eq $admin) { $errors.Add('Authentication.Entra.InitialSuperAdmin is missing.') }
        ...
        $adminEmailValue = Get-JsonValue $admin 'NotificationEmail'
        ...
        if (-not (Test-Email $adminUpn) -or -not (Test-Email $adminEmailValue)) { $errors.Add('InitialSuperAdmin UPN/notification email is invalid.') }
        if ([string]::IsNullOrWhiteSpace([string]$adminDisplay)) { $errors.Add('InitialSuperAdmin.DisplayName is missing.') }
        if ((Test-NonEmptyGuid $adminTenant) -and -not (@($tenantIds | ForEach-Object { [string]$_ }) -icontains [string]$adminTenant)) { $errors.Add('InitialSuperAdmin.TenantId is not in AllowedTenantIds.') }
    }
    ...
    if (-not (Compare-PathValue $keysPath $ExpectedKeysPath) -or -not (Test-Path -LiteralPath $keysPath -PathType Container)) { $errors.Add('Application.DataProtectionKeysPath is not the usable canonical keys path.') }
    if (-not (Compare-PathValue $logsPath $ExpectedLogsPath) -or -not (Test-Path -LiteralPath $logsPath -PathType Container)) { $errors.Add('Application.LogPath is not the usable canonical logs path.') }
    ...
    $domainsValue = Get-JsonValue $emailSafety 'AllowedRecipientDomains'
    if ($null -ne $domainsValue) {
        $domains = @($domainsValue)
        $domainSet = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        foreach ($domain in $domains) {
            if (-not (Test-RecipientDomain $domain)) { $errors.Add('EmailSafety.AllowedRecipientDomains contains an invalid domain.') }
            elseif (-not $domainSet.Add([string]$domain)) { $errors.Add('EmailSafety.AllowedRecipientDomains contains a duplicate domain.') }
        }
    }

    if (-not (Test-IntegerRange (Get-JsonValue $rateLimit 'PermitLimit') 1 10000 $false)) { $errors.Add('PatronLoginRateLimit.PermitLimit is invalid.') }
    if (-not (Test-IntegerRange (Get-JsonValue $rateLimit 'WindowSeconds') 1 86400 $false)) { $errors.Add('PatronLoginRateLimit.WindowSeconds is invalid.') }

    $canonicalSchedules = [ordered]@{
    ...
    if (-not (Test-ExactKeys $schedules @($canonicalSchedules.Keys))) { $errors.Add('Hangfire.Schedules keys do not match the required case-sensitive shape.') }
    else {
        foreach ($name in $canonicalSchedules.Keys) {
            if (-not ([string](Get-JsonValue $schedules $name)).Equals([string]$canonicalSchedules[$name], [StringComparison]::Ordinal)) { $errors.Add("Hangfire.Schedules.$name does not match the canonical test-host schedule.") }
        }
    }

    if (-not (Test-Limit (Get-JsonValue $processing 'Default') $true)) { $errors.Add('Hangfire.ProcessingLimits.Default is invalid.') }
    if (-not (Test-Limit (Get-JsonValue $processing 'Timeouts') $false)) { $errors.Add('Hangfire.ProcessingLimits.Timeouts is invalid.') }
    ...
    if (-not (Test-ExactKeys $queues $requiredQueues)) { $errors.Add('Hangfire.ProcessingLimits.Queues keys do not match the required case-sensitive shape.') }
    else {
        foreach ($name in $requiredQueues) {
            if (-not (Test-Limit (Get-JsonValue $queues $name) $false)) { $errors.Add("Hangfire.ProcessingLimits.Queues.$name is invalid.") }
        }
    }
...
function Test-DeploymentConfiguration([string] $Path, [object] $Expected) {
    ...
    foreach ($name in @('IisSiteName', 'IisAppPoolName')) {
        $actualValue = [string](Get-JsonValue $actual $name)
        $expectedValue = [string](Get-JsonValue $Expected $name)
        if ([string]::IsNullOrWhiteSpace($actualValue) -or -not $actualValue.Equals($expectedValue, [StringComparison]::OrdinalIgnoreCase)) { $errors.Add("$name is stale or missing.") }
    }
    ...
    $expectedSql = [string](Get-JsonValue $Expected 'AsapDatabaseConnectionString')
    foreach ($name in @('AsapDatabaseConnectionString', 'HangfireDatabaseConnectionString')) {
        if (-not (Test-CanonicalConnectionString (Get-JsonValue $actual $name) $expectedSql)) { $errors.Add("$name is stale or is not the canonical bootstrap-owned connection string.") }
    }

    foreach ($name in @('SqlPackagePath', 'SqlCmdPath')) {
        if (-not (Test-EquivalentToolPath ([string](Get-JsonValue $actual $name)) ([string](Get-JsonValue $Expected $name)))) { $errors.Add("$name is stale or missing.") }
    }
    return $errors
}

if ($ContractSelfTest) {
    $selfTestRoot = Join-Path ([IO.Path]::GetTempPath()) ("asap-bootstrap-contract-" + [guid]::NewGuid().ToString('N'))
    try {
        New-Item -ItemType Directory -Force -Path $selfTestRoot | Out-Null
        $toolPath = (Get-Process -Id $PID).Path
        $connectionString = 'Server=contract-test;Database=AsapTest;Integrated Security=True;Encrypt=True;TrustServerCertificate=False'
        $expected = [ordered]@{
            IisSiteName = 'ASAP-Test'
            IisAppPoolName = 'ASAP-Test'
            DeploymentPath = Join-Path $selfTestRoot 'web'
            StagingRoot = Join-Path $selfTestRoot 'staging'
            BackupRoot = Join-Path $selfTestRoot 'backups'
            ExternalApplicationConfigPath = Join-Path $selfTestRoot 'test-app.json'
            ReadinessUrl = 'https://example.invalid/health/ready'
            AsapDatabaseConnectionString = $connectionString
            HangfireDatabaseConnectionString = $connectionString
            SqlPackagePath = $toolPath
            SqlCmdPath = $toolPath
        }
        if ((Get-JsonValue $expected 'IisSiteName') -ne 'ASAP-Test') { throw 'Ordered dictionary lookup regression.' }
        $configPath = Join-Path $selfTestRoot 'test-deployment.json'
        Write-Json $configPath $expected
        $errors = @(Test-DeploymentConfiguration $configPath $expected)
        if ($errors.Count -ne 0) { throw "Fresh deployment configuration failed validation: $($errors -join ' ')" }
        Write-Host 'ASAP test-host bootstrap contract self-test passed.'
        exit 0
    }
    finally { Remove-Item -LiteralPath $selfTestRoot -Recurse -Force -ErrorAction SilentlyContinue }
}
...
function Get-ServiceIdentity([object] $RunnerService) {
    ...
    if ($ServiceAccount -and -not $detected.Equals($ServiceAccount, [StringComparison]::OrdinalIgnoreCase)) { Add-Result 'NEEDS ATTENTION' 'Runner identity' "Runner uses '$detected' but override specifies '$ServiceAccount'." }
...
function Test-RootAce([Security.AccessControl.DirectorySecurity] $Acl, [Security.Principal.SecurityIdentifier] $Sid, [Security.AccessControl.FileSystemRights] $Rights) {
...
function Set-CanonicalRootAcl([string] $Path, [string] $Account) {
    $inheritance = [Security.AccessControl.InheritanceFlags]::ContainerInherit -bor [Security.AccessControl.InheritanceFlags]::ObjectInherit
    $allow = [Security.AccessControl.AccessControlType]::Allow
    $acl = [Security.AccessControl.DirectorySecurity]::new()
    $acl.SetAccessRuleProtection($true, $false)
    $acl.SetOwner([Security.Principal.NTAccount]::new('BUILTIN\Administrators'))
    $acl.AddAccessRule([Security.AccessControl.FileSystemAccessRule]::new('NT AUTHORITY\SYSTEM', [Security.AccessControl.FileSystemRights]::FullControl, $inheritance, [Security.AccessControl.PropagationFlags]::None, $allow))
    $acl.AddAccessRule([Security.AccessControl.FileSystemAccessRule]::new('BUILTIN\Administrators', [Security.AccessControl.FileSystemRights]::FullControl, $inheritance, [Security.AccessControl.PropagationFlags]::None, $allow))
    $acl.AddAccessRule([Security.AccessControl.FileSystemAccessRule]::new($Account, [Security.AccessControl.FileSystemRights]::Modify, $inheritance, [Security.AccessControl.PropagationFlags]::None, $allow))
    Set-Acl -LiteralPath $Path -AclObject $acl
}

function Repair-ManagedChildAcl([string] $Path) {
...
function Test-FilesystemPermissions([string] $RootPath, [Security.Principal.SecurityIdentifier] $ServiceSid, [string[]] $ManagedPaths) {
    $errors = [System.Collections.Generic.List[string]]::new()
    try { $rootAcl = Get-Acl -LiteralPath $RootPath } catch { $errors.Add($_.Exception.Message); return $errors }

    $systemSid = [Security.Principal.SecurityIdentifier]::new('S-1-5-18')
    $adminSid = [Security.Principal.SecurityIdentifier]::new('S-1-5-32-544')
    if (-not $rootAcl.AreAccessRulesProtected) { $errors.Add('Root ACL inheritance is not protected.') }
    try { $ownerSid = ([Security.Principal.NTAccount]::new($rootAcl.Owner)).Translate([Security.Principal.SecurityIdentifier]) } catch { $ownerSid = $null }
    if ($null -eq $ownerSid -or $ownerSid.Value -ne $adminSid.Value) { $errors.Add('Root owner is not BUILTIN\Administrators.') }
    if (-not (Test-RootAce $rootAcl $systemSid ([Security.AccessControl.FileSystemRights]::FullControl))) { $errors.Add('SYSTEM lacks inheritable FullControl on the root.') }
    if (-not (Test-RootAce $rootAcl $adminSid ([Security.AccessControl.FileSystemRights]::FullControl))) { $errors.Add('BUILTIN\Administrators lacks inheritable FullControl on the root.') }
    if (-not (Test-RootAce $rootAcl $ServiceSid ([Security.AccessControl.FileSystemRights]::Modify))) { $errors.Add('Service identity lacks inheritable Modify on the root.') }

    $allowed = @($systemSid.Value, $adminSid.Value, $ServiceSid.Value)
    foreach ($rule in $rootAcl.GetAccessRules($true, $false, [Security.Principal.SecurityIdentifier])) {
        if ($rule.AccessControlType -ne [Security.AccessControl.AccessControlType]::Allow -or $allowed -notcontains $rule.IdentityReference.Value) { $errors.Add("Unexpected explicit root ACL entry for $($rule.IdentityReference.Value).") }
    }

    foreach ($path in $ManagedPaths) {
        if (-not (Test-Path -LiteralPath $path)) { continue }
        $acl = Get-Acl -LiteralPath $path
        if ($acl.AreAccessRulesProtected) { $errors.Add("$path has protected ACL inheritance.") }
        $explicit = @($acl.GetAccessRules($true, $false, [Security.Principal.SecurityIdentifier]))
        if ($explicit.Count -gt 0) { $errors.Add("$path has explicit ACL entries instead of inheriting the managed root ACL.") }
    }
    return $errors
}

function Ensure-FilesystemPermissions([string] $RootPath, [string] $Account, [Security.Principal.SecurityIdentifier] $ServiceSid, [string[]] $ManagedPaths) {
    if (-not $ValidateOnly) {
        Set-CanonicalRootAcl $RootPath $Account
        foreach ($path in $ManagedPaths) { Repair-ManagedChildAcl $path }
    }

    $errors = @(Test-FilesystemPermissions $RootPath $ServiceSid $ManagedPaths)
    if ($errors.Count -eq 0) { Add-Result 'PASS' 'Filesystem permissions' "Restricted inheritable ACL is effective throughout the managed tree under $RootPath." }
...
function Ensure-Group([string] $GroupSid, [string] $Label, [string] $Account, [Security.Principal.SecurityIdentifier] $AccountSid, [bool] $ReportOnly) {
    ...
    if ($ReportOnly) { Add-Result 'INFO' $Label "$Account is not a member of $($group.Name) because -SkipLocalAdministrator was selected. Confirm equivalent delegated IIS lifecycle rights separately; this script does not verify them."; return }
    if ($ValidateOnly) { Add-Result 'NEEDS ATTENTION' $Label "$Account is not a member of $($group.Name)."; return }
...
function Test-CertificateRead([Security.Cryptography.X509Certificates.X509Certificate2] $Certificate, [Security.Principal.SecurityIdentifier] $ServiceSid) {
    ...
    foreach ($rule in $acl.GetAccessRules($true, $true, [Security.Principal.SecurityIdentifier])) {
        if ($rule.AccessControlType -eq [Security.AccessControl.AccessControlType]::Allow -and $rule.IdentityReference.Value -eq $ServiceSid.Value) { $combined = $combined -bor $rule.FileSystemRights }
    }
...
$deploymentConfig = [ordered]@{
    ...
    ExternalApplicationConfigPath = $appConfigPath
...
$managedPaths = @($webRoot, $stagingRoot, $backupRoot, $configRoot, $keysRoot, $logsRoot, $appConfigPath, $deploymentConfigPath)
if (Test-Path -LiteralPath $root -PathType Container) { Ensure-FilesystemPermissions $root $identity.Name $identity.Sid $managedPaths }
...
else { Write-Host 'Host bootstrap checks passed. Confirm SQL authorization and any explicitly delegated IIS lifecycle rights before enabling the GitHub deployment gate.' }
